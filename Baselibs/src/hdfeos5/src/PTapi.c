...
HE5_PTattach(hid_t fid, const char *pointname)
{
  ...
  if (status == SUCCEED)
	...
	npointopen += HE5_PTXPoint[i].active;
	  ...
	  for( i = 0; i < nlevels; i++ )
	    {
	      HE5_PTXPoint[ emptyslot ].level[ i ].name =  (char *)calloc( (strlen(tmpdata.level[i].name)+1), sizeof(char));
...
|  ntype          hid_t               attribute HDF numbertype                |
...
-----------------------------------------------------------------------------*/
herr_t 
HE5_PTwriteattr(hid_t pointID, const char *attrname, hid_t ntype, hsize_t count[],void * datbuf)
{
  herr_t     status     = FAIL;/* routine return status variable */
...
HE5_PTlevelinfo(hid_t pointID, int level, HE5_CmpDTSinfo *info)
{
  ...
  for( i = 0; i < info->nfields; i++)
      ...
      if (mdt == FAIL)
	{
      ...
      /* Get field data class */
      /* -------------------- */
      info->dclass[i] = H5Tget_class(mdt);
      if (info->dclass[i] == H5T_NO_CLASS)
	{
	  sprintf(errbuf, "Cannot get the field class.\n");
	  H5Epush(__FILE__, "HE5_PTlevelinfo", __LINE__, H5E_ARGS, H5E_BADVALUE, errbuf);
      ...
      if (info->dclass[i] == H5T_ARRAY)
	{
	  /* Get the field rank */
	  /* ------------------ */
	  info->rank[i]  = H5Tget_array_ndims(mdt);
	  if (info->rank[i] == FAIL)
	    {
	      sprintf(errbuf, "Cannot get the field rank.\n");
	      H5Epush(__FILE__, "HE5_PTlevelinfo", __LINE__, H5E_ARGS, H5E_BADVALUE, errbuf);
	  ...
	  /* Get the field dimension sizes */
	  /* ----------------------------- */
	  status = H5Tget_array_dims(mdt, dims, NULL);
	  if (status == FAIL)
	    {
	      sprintf(errbuf, "Cannot get the field dimension sizes.\n");
	      H5Epush(__FILE__, "HE5_PTlevelinfo", __LINE__, H5E_ARGS, H5E_BADVALUE, errbuf);
...
-----------------------------------------------------------------------------*/
herr_t 
HE5_PTwritegrpattr(hid_t pointID, const char *attrname, hid_t ntype, hsize_t count[],void *datbuf)
{
  herr_t     status     = FAIL;/* routine return status variable */
...
-----------------------------------------------------------------------------*/
herr_t 
HE5_PTwritelocattr(hid_t pointID, const char *levelname, const char *attrname, hid_t ntype, hsize_t count[],void *datbuf)
{
  herr_t     status     = FAIL;/* routine return status variable */
...
HE5_PTwriteattrF(int pointID, const char *attrname, int ntype, long fortcount[], void *datbuf)
{
  ...
  if(numbertype == FAIL)
    {
      H5Epush(__FILE__, "HE5_PTwriteattrF", __LINE__, H5E_DATATYPE, H5E_NOTFOUND, "Error calling HE5_EHconvdatatype() from FORTRAN wrapper.");
  ...
  if(numbertype == FAIL)
    {
      ...
      H5Epush(__FILE__, "HE5_PTwritegrpattrF", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      free(errbuf);
      return(FAIL);
    }
  ...
  if(numbertype == FAIL)
    {
      ...
      H5Epush(__FILE__, "HE5_PTwritelocattrF", __LINE__, H5E_FUNC, H5E_CANTINIT, errbuf);
      HE5_EHprint(errbuf, __FILE__, __LINE__);
      return(FAIL);
    }
...
HE5_PTinqdatatypeF(int pointID, char *fieldname, char *attrname, int fieldgroup, int *Type, int *Class, int *Order, long *size)
{
  ...
  /* (still works with '0'                */
  /* ------------------------------------ */
  if( *fieldname == '0' ) { *fieldname = (char)NULL; }
  if( *attrname  == '0' ) { *attrname  = (char)NULL; }
