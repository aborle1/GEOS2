...
DEGREE VERSION OF THE SEARCHING FUNCTION
    ...
    PGSt_integer numPoints)   		 /*number points in orderedArray*/
{
    ...
    /*static PGSt_integer pixDegreeOrig;*/  /*pixels per degree for original 
					  resolution*/
    ...
    {
	/*record the "original" geopositioning pixels per Degree ratio*/
	/*pixDegreeOrig = subsetInfo -> pixPerDegree;*/
    ...
    /*Base case of recursion function-- if only one resolution left in
      ...
      one in any of the subgrids. I do this initial check  for error reporting.
      This way the user knows that there are still some fill values in the
	...
	for (j = 0; j < numSubgridCover; j++)
	{
	    ...
	    /*get the number of data points to be extracted from the present
	      ...
	      orderedArray, to the present point you will be analyzing.  That
		...
		statusSmf = PGS_SMF_SetDynamicMsg(PGSDEM_E_HDFEOS,
						  ...
						  "PGS_DEM_PGS_DEM_RecursiveSearchDeg()");
	    ...
	    /* Get the number of points requested from this subgrid. this is the
		...
		/*Calculate global pixels in the new resolution--
		  these pixels are NOT valid for the internal GRID. Automaticly
		  nearest neighbor interpolation*/
		latNearest = 
		  (int32) PGSm_DEM_LatToPixel(dataPoints[orderedArray[i]] ->
					      positionOrig[0], subsetInfo);
		

		lonNearest = 
		  (int32) PGSm_DEM_LonToPixel(dataPoints[orderedArray[i]] ->
					      positionOrig[1], subsetInfo);
	    ...
	    /*NOTE-- to speed up, could co-opt code from
	      GDgetpixelvalues, don't need some of the checks for
	      merged fields, etc..*/
	...
	/*Before returning, check if all the fill values have been filled, and
	...
	for (j = 0; j < numSubgridCover; j++)
	{
	    ...
	    if (subsetInfo -> dataType == DFNT_FLOAT32)
  	    {
		/*first calloc space for output buffer*/
		dataBuffFlt32 = (float32 *) calloc(presentNumPoints, sizeof(float32));
		if (dataBuffFlt32 == NULL)
		{
		    /*ERROR callocing*/
		/*ERROR callocing*/
					     ...
					     previousNumPoints); i++) 
		{
			...
			latNearest = (int32)
			  PGSm_DEM_LatToPixel(dataPoints[orderedArray[i]] ->
					      positionOrig[0], subsetInfoNext);
		
			lonNearest = (int32)
			  PGSm_DEM_LonToPixel(dataPoints[orderedArray[i]] ->
					      positionOrig[1], subsetInfoNext);
	...
	if (numFillPoints == 0)
	{
	    /*Successfully acquired all the data*/
	    /*Check if acquried all the data from one or multiple resolutions.
	...
	statusSmf = PGS_DEM_OrderIndicesSumDeg(orderedArray, numFillPoints,
	...
	statusSmf = PGS_DEM_RecursiveSearchDeg((resolutionList + 1),
					       numResolutions - 1,
					       subsetInfoNext,
					       subsetNext,
					       dataPoints,
