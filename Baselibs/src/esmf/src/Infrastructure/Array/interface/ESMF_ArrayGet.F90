...
!------------------------------------------------------------------------------
    ...
    type(ESMF_Pointer), pointer :: opt_localarrayPtrList(:) ! helper variable
    integer :: len_localarrayPtrList, i ! helper variable
    ...
    if (present(localDeList)) then
      call ESMF_LogWrite("The use of argument 'localDeList' in call " CONCAT/ &
        "ESMF_ArrayGet() is DEPRECATED! Use argument 'localDeToDeMap' " CONCAT/ &
...
type(ESMF_KeywordEnforcer), optional:: keywordEnforcer ! must use keywords below 
 ...
 msg="- farrayPtr typekind does not match Array typekind", & 
 ESMF_CONTEXT, rcToReturn=rc) 
 return 
 endif 
 ...
 if (rank /= 1) then 
 call ESMF_LogSetError(rcToCheck=ESMF_RC_ARG_INCOMP, & 
 msg="- farrayPtr rank does not match Array rank", & 
 ESMF_CONTEXT, rcToReturn=rc) 
 return 
 endif 
 ...
 if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
 call ESMF_LocalArrayGet(localarray, farrayPtr, datacopyflag=ESMF_DATACOPY_REFERENCE, rc=localrc) 
 if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
