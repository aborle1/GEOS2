...
! !INTERFACE: 
 ...
 proxyflag, addflag, replaceflag, relaxflag, rc) 
! 
! !ARGUMENTS: 
 type(ESMF_State), intent(inout) :: state 
 type(ESMF_Array), intent(in) :: array 
 ...
 logical, intent(in), optional :: relaxflag 
 integer, intent(out), optional :: rc 
!EOPI 
!------------------------------------------------------------------------------ 
 ...
 addflag=addflag, replaceflag=replaceflag, relaxflag=relaxflag, & 
...
!------------------------------------------------------------------------------
      ...
      call ESMF_StateClsAdd (state%statep, routehandles=temp_list, &
          addflag=.true., rc=localrc)
      if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
                    ESMF_CONTEXT, rcToReturn=rc)) return
      if (present(rc)) rc = ESMF_SUCCESS
...
!
! !INTERFACE:
! subroutine ESMF_StateAdd(state, <itemList>, rc)
!
! !ARGUMENTS:
! type(ESMF_State), intent(inout) :: state
...
!------------------------------------------------------------------------------
#undef ESMF_METHOD
#define ESMF_METHOD "ESMF_StateAddRpRouteHandleList"
!BOPI
! !IROUTINE: ESMF_StateAdd - Add or replace a list of RouteHandles to a State
!
! !INTERFACE:
      ! Private name; call using ESMF_StateAddReplace()
...
!
! !DESCRIPTION:
! Add or replace multiple {\tt ESMF\_RouteHandle}s to an {\tt ESMF\_State}.
!
! The arguments are:
! \begin{description}
! \item[state]
! An {\tt ESMF\_State} object.
! \item[routehandleList]
! The list (Fortran array) of {\tt ESMF\_RouteHandle}s to be added and replaced.
! This is a reference only; when
! the {\tt ESMF\_State} is destroyed the objects contained in it will
! not be destroyed. Also, the {\tt ESMF\_RouteHandle}s cannot be safely
! destroyed before the {\tt ESMF\_State} is destroyed.
...
! !INTERFACE:
      ...
      subroutine ESMF_StateGetInfo(state, &
            keywordEnforcer, itemSearch, nestedFlag, stateintent, &
...
! if (present (itemCount) &
! .or. present (itemNameList) &
! .or. present (itemTypeList)) then
! call ESMF_StateGetInfoWorker (stypep, localnestedFlag, nestedPath='', &
! itemSearch=itemSearch, itemCount=itemCount, &
! names=itemNameList, types=itemTypeList, rc=localrc)
! if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
! ESMF_CONTEXT, rcToReturn=rc)) return
! end if
   ...
   call ESMF_ContainerGet (sp%stateContainer, &
              itemCount=sipcount, itemList=siwrap, rc=localrc)
   if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, &
       ESMF_CONTEXT, rcToReturn=rc)) return
          do, i1 = 1, sipcount
            ...
            sip => siwrap(i1)%si
            if (.not. present (itemSearch)) then
              itemNameList(ilpos) = prefix CONCAT/ sip%namep
              ilpos = ilpos + 1
            else
            ...
            if (sip%otype%ot == ESMF_STATEITEM_STATE%ot &
              ...
              call itemNameWorker (sip%datap%spp, &
                  prefix=prefix CONCAT/ trim (sip%namep) CONCAT/ '/')
            end if
          end do
          if (associated (siwrap)) &
            deallocate (siwrap, stat=memstat1)
