...
! !INTERFACE: 
 ...
 addflag, replaceflag, relaxflag, proxyflag, rc) 
! 
! !ARGUMENTS: 
 type(ESMF_StateClass), pointer :: stypep 
...
!EOPI 
 ...
 logical :: localrelaxflag ! local relaxed flag 
 ...
 localrelaxflag = .false. 
 if (present (relaxflag)) then 
 localrelaxflag = relaxflag 
 end if 
 ...
 if (exists) then 
 ! can do addrep or replace. Or add w/relaxed 
 if (.not. localrepflag .and. (localaddflag .and. .not. localrelaxflag)) then 
 errmsg = "existing item " CONCAT/ trim (itemname) CONCAT/ " not replacable" 
 ...
 ! can do add or addrep. Or replace w/relaxed 
 if (.not. localaddflag .and. (localrepflag .and. .not. localrelaxflag)) then 
 errmsg = "existing item " CONCAT/ trim (itemname) CONCAT/ " not found" 
 if (ESMF_LogFoundError (ESMF_RC_ARG_INCOMP, & 
 msg = trim (errmsg), & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
 end if 
...
!EOPI
      ...
      ! This function is only called internally, so technicaly we do not need
