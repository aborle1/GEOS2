...
! Local derived type aliases
  ...
  character(len=ESMF_MAXSTR)          :: REPLAY_FILE1
  character(len=ESMF_MAXSTR)          :: REPLAY_FILE2
  character(len=ESMF_MAXSTR)          :: FILETMPL
  character(len=ESMF_MAXSTR)          :: cremap
  character(len=ESMF_MAXSTR)          :: fixwind

  type(ESMF_FieldBundle), save        :: RBUNDLE1
  type(ESMF_FieldBundle), save        :: RBUNDLE2
  type(ESMF_Time),        save        :: TBUNDLE1
  type(ESMF_Time),        save        :: TBUNDLE2
  type(ESMF_Time)                     :: REPLAY_TIME2
  type(ESMF_Time)                     :: REPLAY_TIME1
  ...
  type(ESMF_TimeInterval)             :: Analysis_Interval
  integer                             :: Analysis_Frequency
...
! **********************************************************************
    ...
    call MAPL_GetResource(MAPL,Analysis_Frequency, Label="ANALYSIS_FREQUENCY:", default=-999, rc=STATUS )
    VERIFY_(STATUS)

    if( Analysis_Frequency == -999 ) then
        REPLAY_TIME1 = currTime
    else
        call ESMF_TimeIntervalSet( Analysis_Interval, S=Analysis_Frequency, rc=STATUS )
        VERIFY_(STATUS)
        REPLAY_TIME1 = currTime + ( Analysis_Interval / 2 )
        call ESMF_TimeGet( REPLAY_TIME1, YY = CUR_YY, &
                                         MM = CUR_MM, &
                                         DD = CUR_DD, &
                                          H = CUR_H , &
                                          M = CUR_M , &
                                         ...
                                         rc = STATUS  )
        VERIFY_(STATUS)
        ANA_H = Analysis_Frequency/3600
        CUR_H = ANA_H*(CUR_H/ANA_H)
        call ESMF_TimeSet( REPLAY_TIME1, YY = CUR_YY, &
                                         MM = CUR_MM, &
                                         DD = CUR_DD, &
                                          H = CUR_H , &
                                          M = 0     , &
    ...
    call ESMF_TimeGet(REPLAY_TIME1, timeString=DATE, RC=STATUS)
    VERIFY_(STATUS)
    call strToInt(DATE, nymd1, nhms1)
    call ESMF_CFIOstrTemplate ( REPLAY_FILE1, FILETMPL, 'GRADS', nymd=nymd1, nhms=nhms1, stat=STATUS )
    VERIFY_(STATUS)

    if( currTime == REPLAY_TIME1 ) then
        fac1 = 1.0
        fac2 = 0.0
        if(MAPL_AM_I_ROOT() ) then
           print *, 'Current nymd: ',nymd,'  nhms: ',nhms,'  FAC: ',fac1
           print *
        endif
    elseif( currTime < REPLAY_TIME1 ) then
        REPLAY_TIME2 = REPLAY_TIME1 - Analysis_Interval
        call ESMF_TimeGet(REPLAY_TIME2, timeString=DATE, RC=STATUS)
        VERIFY_(STATUS)
        call strToInt(DATE, nymd2, nhms2)
        call ESMF_CFIOstrTemplate ( REPLAY_FILE2, FILETMPL, 'GRADS', nymd=nymd2, nhms=nhms2, stat=STATUS )
        VERIFY_(STATUS)
        fac1 = ( currTime - REPLAY_TIME2 )/( REPLAY_TIME1 - REPLAY_TIME2 )
        fac2 = 1.0 - fac1
        if(MAPL_AM_I_ROOT() ) then
           print *, 'Previous nymd: ',nymd2,'  nhms: ',nhms2,'  FAC: ',fac2
           print *, 'Current  nymd: ',nymd ,'  nhms: ',nhms
           print *, 'Next     nymd: ',nymd1,'  nhms: ',nhms1,'  FAC: ',fac1
           print *
        endif
    else
        REPLAY_TIME2 = REPLAY_TIME1 + Analysis_Interval
        ...
        call ESMF_CFIOstrTemplate ( REPLAY_FILE2, FILETMPL, 'GRADS', nymd=nymd2, nhms=nhms2, stat=STATUS )
        VERIFY_(STATUS)
        fac1 = ( REPLAY_TIME2 - currTime )/( REPLAY_TIME2 - REPLAY_TIME1 )
        fac2 = 1.0 - fac1
        if(MAPL_AM_I_ROOT() ) then
           print *, 'Prevous nymd: ',nymd1,'  nhms: ',nhms1,'  FAC: ',fac1
           print *, 'Current nymd: ',nymd ,'  nhms: ',nhms
           print *, 'Next    nymd: ',nymd2,'  nhms: ',nhms2,'  FAC: ',fac2
           print *
        endif
    endif

    call CFIO_Open       ( REPLAY_FILE1, 1, fid, STATUS )
    VERIFY_(STATUS)
    call CFIO_DimInquire ( fid, IMana_World, JMana_world, LM, nt, nvars, natts, STATUS )
    VERIFY_(STATUS)
...
! *****************************************************************************
    ...
    RBUNDLE1 = ESMF_FieldBundleCreate( RC=STATUS)
    VERIFY_(STATUS)
    call ESMF_FieldBundleSet(RBUNDLE1, grid=GRIDana, rc=status)
    VERIFY_(STATUS)
    call MAPL_CFIORead ( REPLAY_FILE1, REPLAY_TIME1, RBUNDLE1, RC=status)
    VERIFY_(STATUS)
    call ESMF_FieldBundleGet ( RBUNDLE1, fieldCount=NQ, RC=STATUS )
    VERIFY_(STATUS)
...
! ------------------------------
    ...
    do ni = 1, nq
       call ESMF_FieldBundleGet(RBUNDLE1, ni, Field, __RC__ )
...
!   Clean up
!   --------
    call MAPL_FieldBundleDestroy ( RBUNDLE1, RC=STATUS)
    VERIFY_(STATUS)
    deallocate(dp)
    deallocate(gptr2d)
    deallocate(gptr3d)
...
! *****************************************************************************
    ...
    if( LBUNDLE1 ) then
        ...
        call MAPL_CFIORead ( REPLAY_FILE1, REPLAY_TIME1, RBUNDLE1 , RC=status)
        VERIFY_(STATUS)
        TBUNDLE1 = REPLAY_TIME1
        LBUNDLE1 = .FALSE.
    else if( TBUNDLE1 .ne. REPLAY_TIME1 ) then
        call MAPL_CFIORead ( REPLAY_FILE1, REPLAY_TIME1, RBUNDLE1 , RC=status)
        VERIFY_(STATUS)
        TBUNDLE1 = REPLAY_TIME1
    endif
    ...
    if( fac1.ne.1.0 ) then 
        if( LBUNDLE2 ) then 
            RBUNDLE2 = ESMF_FieldBundleCreate( RC=STATUS)
            VERIFY_(STATUS)
            call ESMF_FieldBundleSet(RBUNDLE2, grid=GRIDana, rc=status)
            VERIFY_(STATUS)
            call MAPL_CFIORead ( REPLAY_FILE2, REPLAY_TIME2, RBUNDLE2 , RC=status)
            VERIFY_(STATUS)
            TBUNDLE2 = REPLAY_TIME2
            LBUNDLE2 = .FALSE.
        else if ( TBUNDLE2 .ne. REPLAY_TIME2 ) then
            call MAPL_CFIORead ( REPLAY_FILE2, REPLAY_TIME2, RBUNDLE2 , RC=status)
            VERIFY_(STATUS)
            TBUNDLE2 = REPLAY_TIME2
        endif
    ...
    if( .not.allocated( rnames ) ) then
         ...
         VERIFY_(STATUS)
         call ESMF_FieldBundleGet ( RBUNDLE1, fieldNameList=RNAMES, rc=STATUS )
         VERIFY_(STATUS)
         if( first ) then
             if(MAPL_AM_I_ROOT() ) then
             print *
...
!--------------------
        ...
        do k=1,nq
           if( match('ts',REPLAY_TS,rnames(k)) ) then
               call ESMFL_BundleGetPointertoData(RBUNDLE1,trim(rnames(k)),ptr2d, RC=STATUS)
               if(STATUS==ESMF_SUCCESS) then
                          ts_ana =  ptr2d
                          if( fac1.ne.1.0 ) then
                              call ESMFL_BundleGetPointertoData(RBUNDLE2,trim(rnames(k)),ptr2d, RC=STATUS)
                              VERIFY_(STATUS)
                              ts_ana =  fac1*ts_ana + fac2*ptr2d
                          endif
                          FOUND  = .true.
                          exit
               endif
...
!-----------------------
        ...
        do k=1,nq
           if( match('dp',REPLAY_DP,rnames(k)) ) then
               call ESMFL_BundleGetPointertoData(RBUNDLE1,trim(rnames(k)),ptr3d, RC=STATUS)
               if(STATUS==ESMF_SUCCESS) then
                          dp_ana =  ptr3d
                          if( fac1.ne.1.0 ) then
                              call ESMFL_BundleGetPointertoData(RBUNDLE2,trim(rnames(k)),ptr3d, RC=STATUS)
                              VERIFY_(STATUS)
                              dp_ana =  fac1*dp_ana + fac2*ptr3d
                          endif
                          FOUND  = .true.
                          exit
               endif
...
! **********************************************************************
    ...
    if (USE_SPECFILT .and. (L_REPLAY_U .or. L_REPLAY_V) ) then
...
! -----------
    ...
    if (USE_SPECFILT .and. L_REPLAY_T) then
...
! --------
    ...
    if (USE_SPECFILT .and. L_REPLAY_QV) then
        call Spectrans_ScalarPar (im,jm,lm,ptr3d,JCAP,GRIDana,RC=STATUS)
        VERIFY_(STATUS)
    endif
    if (do_transforms) then
...
! -----
    ...
    if (USE_SPECFILT .and. L_REPLAY_O3) then
        call Spectrans_ScalarPar (im,jm,lm,ptr3d,JCAP,GRIDana,RC=STATUS)
        VERIFY_(STATUS)
    endif
    if (do_transforms) then
...
! ---
    ...
    if (USE_SPECFILT .and. L_REPLAY_P) then
...
#if 0
  ! ----------------------------------------------------
  ! Simple Nudging based on using Predictor Methodology:
  ! Create Analysis State on Background Grid by adding 
  ! Analysis Increments to Background IMPORTS
  ! ----------------------------------------------------
    ...
    call MAPL_GetPointer( import, uptr3d, 'QV',    RC=STATUS)
    VERIFY_(STATUS)
    dq = uptr3d + dq

    call MAPL_GetPointer( import, vptr3d,  'TV',   RC=STATUS)
    VERIFY_(STATUS)
    vptr3d = vptr3d/(1.0+eps*uptr3d)  ! Construct Background Dry Temperature
        dt = vptr3d + dt

    call MAPL_GetPointer( import, uptr3d,  'O3PPMV',    RC=STATUS)
    VERIFY_(STATUS)
    do3 = uptr3d + do3

    call MAPL_GetPointer( import, ptr3d,  'DELP',    RC=STATUS)
    VERIFY_(STATUS)
    allocate( vptr3d(IMbkg,JMbkg,0:LM),stat=STATUS)
    vptr3d(:,:,0) = ak(0)
    do L=1,lm
    vptr3d(:,:,L) = vptr3d(:,:,L-1) + ptr3d(:,:,L)
    end do
    dple = vptr3d + dple  ! NUDGING, Add Increment to Background PLE IMPORT
    deallocate( vptr3d )
#endif
...
! ----------------------------------------------------------
    ...
    if (USE_SPECFILT .and. L_REPLAY_TS) then
