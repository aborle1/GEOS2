...
! !USES:
   ...
   use fv_grid_tools_mod,  only: rarea, area, globalarea, globalsum, dx, dy, dxc, dyc, &
                          dxa, dya, rdxa, rdya, rotate_winds, grid_type, &
                          get_unit_vector
...
!-----------------------------------------------------------------------
 subroutine FV_Setup(GC,LAYOUT_FILE, RC)
  ...
  use fv_control_mod, only : uniform_ppm, gmao_cubic, remap_t,  inline_q, z_tracer, fv_debug
  ...
  use fv_control_mod, only : fill, fill_dp, fill_wz
  use fv_control_mod, only : n_sponge, d2_bg_k1, d2_bg_k2
  use test_cases_mod, only : test_case, init_double_periodic
  use fv_control_mod, only : do_schmidt, stretch_fac, target_lat, target_lon
  use fv_control_mod, only : SuperSat
...
!
  ...
  call ESMF_ConfigGetAttribute( cf,   do_schmidt, label= 'do_schmidt:' , default= do_schmidt , rc = rc )
  call ESMF_ConfigGetAttribute( cf,  stretch_fac, label='stretch_fac:' , default=stretch_fac , rc = rc )
  call ESMF_ConfigGetAttribute( cf,   target_lat, label= 'target_lat:' , default= target_lat , rc = rc )
  call ESMF_ConfigGetAttribute( cf,   target_lon, label= 'target_lon:' , default= target_lon , rc = rc )
...
! Fill negative tracers
  call ESMF_ConfigGetAttribute( cf, fill, label='fill:' , default= .false., rc = rc )
...
! Note: iad=2 or 4 appear to be more stable than other options
  ...
  call ESMF_ConfigGetAttribute( cf, SuperSat , label='SuperSat:' , default=SuperSat, rc = rc )
 ...
 !! gmao_cubic=F : Use GFDL remapping schemes in fv_mapz for TE, T or PT
 !! gmao_cubic=T : Use GMAO cubic remapping in fv_mapz for TE, T or PT 
  gmao_cubic=.true.
  call ESMF_ConfigGetAttribute( cf, gmao_cubic, label='gmao_cubic:', default=gmao_cubic, rc = rc )
!
...
subroutine FV_Run (STATE, CLOCK, RC)
  ...
  use fv_control_mod, only : inline_q
...
! Begin
  ...
  ! Be sure we have the correct PHIS and number of tracers for this run
   if (fv_first_run) then
    ...
    ! CHECK INPUT TRACERS, MAKE POSITIVE-DEFINITE
    !do n=1,FV_Atm(1)%ncnst
    !  if (state%vars%tracer(n)%is_r4) then
    !     where( state%vars%tracer(n)%content_r4(:,:,:) < 0.0)
    !            state%vars%tracer(n)%content_r4(:,:,:) = 0.0
    !     endwhere
    !  else
    !     where( state%vars%tracer(n)%content(:,:,:) < 0.0)
 ...
 ! CHECK INPUT TRACERS
   !do n=1,FV_Atm(1)%ncnst
   !   if (state%vars%tracer(n)%is_r4) then
   !      DEBUG_ARRAY = state%vars%tracer(n)%content_r4(:,:,:)
   !   else
   !      DEBUG_ARRAY = state%vars%tracer(n)%content(:,:,:)
   !   endif
   !   call prt_maxmin('R4_IN '//TRIM(state%vars%tracer(n)%tname), DEBUG_ARRAY, isc, iec, jsc, jec, 0, npz, fac1, gid==0)
   !enddo
 ...
 ! CHECK INPUT TRACERS
 !  do n=1,FV_Atm(1)%ncnst
 !     DEBUG_ARRAY = FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,n)
 !     call prt_maxmin('FV_IN '//TRIM(state%vars%tracer(n)%tname), DEBUG_ARRAY, isc, iec, jsc, jec, 0, npz, fac1, gid==0)
 !  enddo

    if (DEBUG .and. inline_q) then
       DEBUG_ARRAY = FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,1)
       call prt_maxmin('FV3 Inline QV ', DEBUG_ARRAY, isc, iec, jsc, jec, 0, npz, fac1, gid==0)
    endif
...
! Copy FV to internal State
 ...
 ! CHECK OUTPUT TRACERS
 !  do n=1,FV_Atm(1)%ncnst
 !     DEBUG_ARRAY = FV_Atm(1)%q(isc:iec,jsc:jec,1:npz,n)
 !     call prt_maxmin('FV_OU '//TRIM(state%vars%tracer(n)%tname), DEBUG_ARRAY, isc, iec, jsc, jec, 0, npz, fac1, gid==0)
 !  enddo
 ...
 !        DEBUG_ARRAY = state%vars%tracer(n)%content(:,:,:)
 !     endif
 !     call prt_maxmin('R4_OU '//TRIM(state%vars%tracer(n)%tname), DEBUG_ARRAY, isc, iec, jsc, jec, 0, npz, fac1, gid==0)
 !  enddo

    ! CHECK TRACERS, ENSURE ALL TRACERS ARE POSITIVE-DEFINITE IF fill=.true. 
    if (FV_Atm(1)%fill) then
     do n=1,FV_Atm(1)%ncnst
       if (state%vars%tracer(n)%is_r4) then
          where( state%vars%tracer(n)%content_r4(:,:,:) < 0.0)
                 state%vars%tracer(n)%content_r4(:,:,:) = 0.0
...
subroutine fv_getEPV(pt, vort, ua, va, epv)
  ...
  real(REAL8), intent(IN)  ::    ua(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz)
  real(REAL8), intent(IN)  ::    va(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz)
  real(REAL8), intent(OUT) ::   epv(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz)

  real(REAL8) :: dz_g(FV_Atm(1)%isd:FV_Atm(1)%ied,FV_Atm(1)%jsd:FV_Atm(1)%jed,1:FV_Atm(1)%npz)

  real(REAL8) :: pt_g(FV_Atm(1)%isd:FV_Atm(1)%ied,FV_Atm(1)%jsd:FV_Atm(1)%jed,1:FV_Atm(1)%npz)
  real(REAL8) :: pt_e(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz+1)

  real(REAL8) :: ua_e(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz+1)
  real(REAL8) :: va_e(FV_Atm(1)%isc:FV_Atm(1)%iec,FV_Atm(1)%jsc:FV_Atm(1)%jec,1:FV_Atm(1)%npz+1)

  real(REAL8) :: vtcorr
  real(REAL8) :: pt_l, pt_r, pt_b, pt_t, dptdx, dptdy, dptdp
  real(REAL8) :: w_l, w_r, w_b, w_t, dwdx, dwdy, dudp, dvdp 

  real(REAL8) :: dudz, dvdz, dptdz, area_dxdz, area_dydz, area_dxdy
  real(REAL8) :: w_im1, w_jm1, w_ip1, w_jp1
  real(REAL8) :: pt_im1, pt_jm1, pt_ip1, pt_jp1
  real(REAL8) :: dz_im1, dz_jm1, dz_ip1, dz_jp1
  real(REAL8) :: vort_i, vort_j, vort_k
  ...
  npz = FV_Atm(1)%npz
   ...
   call mpp_update_domains(pt_g, domain, complete=.true.)
! Get PT/UA/VA at layer edges
   ...
   if (.not. FV_HYDROSTATIC) then
      ...
      call mpp_update_domains(dz_g, domain, complete=.false.)
      call mpp_update_domains(FV_Atm(1)%w,  domain, complete=.true.)
      do k=1,npz
        do j=jsc,jec
          do i=isc,iec
! get all dz and areas needed
             ...
             area_dxdz = dxa(i,j) * 0.5 * ( dz_im1 + dz_ip1 )
             area_dydz = dya(i,j) * 0.5 * ( dz_jm1 + dz_jp1 )
             area_dxdy = area(i,j)
! Get W on center of horizontal-cell edges
             w_im1 = (dxa(i-1,j)*FV_Atm(1)%w(i-1,j,k) + dxa(i  ,j)*FV_Atm(1)%w(i  ,j,k))/(dxa(i-1,j)+dxa(i  ,j))
             w_ip1 = (dxa(i  ,j)*FV_Atm(1)%w(i  ,j,k) + dxa(i+1,j)*FV_Atm(1)%w(i+1,j,k))/(dxa(i  ,j)+dxa(i+1,j))
             w_jm1 = (dya(i,j-1)*FV_Atm(1)%w(i,j-1,k) + dya(i,j  )*FV_Atm(1)%w(i,j  ,k))/(dya(i,j-1)+dya(i,j  ))
             w_jp1 = (dya(i,j  )*FV_Atm(1)%w(i,j  ,k) + dya(i,j+1)*FV_Atm(1)%w(i,j+1,k))/(dya(i,j  )+dya(i,j+1))
! Get PT on center of horizontal-cell edges
             pt_im1 = (dxa(i-1,j)*pt_g(i-1,j,k) + dxa(i  ,j)*pt_g(i  ,j,k))/(dxa(i-1,j)+dxa(i  ,j))
             pt_ip1 = (dxa(i  ,j)*pt_g(i  ,j,k) + dxa(i+1,j)*pt_g(i+1,j,k))/(dxa(i  ,j)+dxa(i+1,j))
             pt_jm1 = (dya(i,j-1)*pt_g(i,j-1,k) + dya(i,j  )*pt_g(i,j  ,k))/(dya(i,j-1)+dya(i,j  ))
             pt_jp1 = (dya(i,j  )*pt_g(i,j  ,k) + dya(i,j+1)*pt_g(i,j+1,k))/(dya(i,j  )+dya(i,j+1))
! d(pt)/dx
             dptdx = (pt_ip1      - pt_im1       )/dxa(i,j)
! d(pt)/dy
             dptdy = (pt_jp1      - pt_jm1       )/dya(i,j)
! d(pt)/dz
             dptdz = (pt_e(i,j,k) - pt_e(i,j,k+1))/dz_g(i,j,k)
! i-component of EPV
             vort_i = (1./area_dydz) * ( va_e(i,j,k+1)*dya(i,j) - va_e(i,j,k)*dya(i,j) - &
                                                 w_jm1*dz_jm1   +       w_jp1*dz_jp1 )
! j-component of EPV 
             vort_j = (1./area_dxdz) * ( ua_e(i,j,k)*dxa(i,j)   - ua_e(i,j,k+1)*dxa(i,j) - &
                                                 w_ip1*dz_ip1   +       w_im1*dz_im1 ) + &
                                         2.*MAPL_OMEGA*COS(FV_Atm(1)%agrid(i,j,2))
! k-component of EPV (pre-computed and passed in to this subroutine)
             vort_k = (vort(i,j,k)+f0(i,j))
...
! d(pt)/dx
             pt_l = (dxa(i-1,j)*pt_g(i-1,j,k) + dxa(i  ,j)*pt_g(i  ,j,k))/(dxa(i-1,j)+dxa(i  ,j))
             pt_r = (dxa(i  ,j)*pt_g(i  ,j,k) + dxa(i+1,j)*pt_g(i+1,j,k))/(dxa(i  ,j)+dxa(i+1,j))
             dptdx = (pt_r-pt_l)/dxa(i,j)
! d(pt)/dy
             pt_b = (dya(i,j-1)*pt_g(i,j-1,k) + dya(i,j  )*pt_g(i,j  ,k))/(dya(i,j-1)+dya(i,j  ))
             pt_t = (dya(i,j  )*pt_g(i,j  ,k) + dya(i,j+1)*pt_g(i,j+1,k))/(dya(i,j  )+dya(i,j+1))
             dptdy = (pt_t-pt_b)/dya(i,j)
! d(pt)/dp
...
! Vorticity Correction to account for changes in theta along isobaric surfaces
             vtcorr = -dvdp*dptdx + dudp*dptdy + (vort(i,j,k)+f0(i,j))*dptdp
