...
contains
   ...
   subroutine get_external_ic( Atm, fv_domain, use_geos_latlon_restart, use_geos_cubed_restart, ntracers )
      ...
      integer, optional,   intent(in)    :: ntracers(6)
      real:: alpha = 0.
      integer i,j,k,nq
...
! delz:
   ...
   subroutine get_geos_cubed_ic( Atm, fv_domain, nq, ntracers )
      ...
      type(fv_atmos_type), intent(inout) :: Atm(:)
      type(domain2d),      intent(inout) :: fv_domain
      integer, intent(in):: nq, ntracers(6)
      ...
      integer :: iq_gmichem0, iq_gmichem1
      integer :: iq_stratchem0, iq_stratchem1
      integer :: iq_tr0, iq_tr1
      integer :: ntiles=6
      logical found
      integer :: header(6)
      ...
      integer :: nmoist, ngocart, npchem, nqmap, ngmichem, nstratchem, ntr
...
!bma added
      ...
      iq_gmichem0=iq_pchem1 +1
      iq_gmichem1=iq_pchem1 +ntracers(4)
      iq_stratchem0=iq_gmichem1 +1
      iq_stratchem1=iq_gmichem1 +ntracers(5)
      iq_tr0=iq_stratchem1 +1
      iq_tr1=iq_stratchem1 +ntracers(6)
      nmoist  = ntracers(1)
      ngocart = ntracers(2)
      npchem  = ntracers(3)
      ngmichem = ntracers(4)
      nstratchem = ntracers(5)
      ntr = ntracers(6)
...
! Horiz Interp for GMICHEM tracers
! is there a gmichem restart file to interpolate?
         if( file_exist("gmichem_internal_restart_in") .and. ntracers(4) > 0 ) then
            if (gid==0) print*, 'Trying to interpolate gmichem_internal_restart_in'

            call MAPL_NCIOGetFileType("gmichem_internal_restart_in",filetype)
            ...
            if (filetype /= 0) then
               ...
               lvar_cnt = 0 
               allocate(gslice_r4(im,jm))
               NCIO = MAPL_NCIOOpen("gmichem_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
               if (nVars /= iq_gmichem1-iq_gmichem0+1) call mpp_error(FATAL,'Wrong number of variables in gmichem file') 
               tileoff = (tile-1)*(jm/ntiles)
            end if

            do ivar=iq_gmichem0,iq_gmichem1
               if (filetype ==0) lvar_cnt=lvar_cnt+1
               do k=1,km
                  if (filetype /= 0) then
                     call parallel_read_file_r4('gmichem_internal_restart_in', npts, is_i,ie_i, js_i,je_i, 1, offset, q0(is_i:ie_i,js_i:je_i,k))
                     call mpp_update_domains(q0(:,:,k), domain_i)
                  else
                     call MAPL_NCIOGetVarName(NCIO,lvar_cnt,vname)
                     call MAPL_VarRead(NCIO,vname,gslice_r4,lev=k)
...
! Horiz Interp for STRATCHEM tracers
! is there a stratchem restart file to interpolate?
         if( file_exist("stratchem_internal_restart_in") .and. ntracers(5) > 0 ) then
            if (gid==0) print*, 'Trying to interpolate stratchem_internal_restart_in'

            call MAPL_NCIOGetFileType("stratchem_internal_restart_in",filetype)
            ...
            if (filetype /= 0) then
               ...
               lvar_cnt = 0 
               allocate(gslice_r4(im,jm))
               NCIO = MAPL_NCIOOpen("stratchem_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
               if (nVars /= iq_stratchem1-iq_stratchem0+1) call mpp_error(FATAL,'Wrong number of variables in stratchem file') 
               tileoff = (tile-1)*(jm/ntiles)
            end if

            do ivar=iq_stratchem0,iq_stratchem1
               if (filetype ==0) lvar_cnt=lvar_cnt+1
               do k=1,km
                  if (filetype /= 0) then
                     call parallel_read_file_r4('stratchem_internal_restart_in', npts, is_i,ie_i, js_i,je_i, 1, offset, q0(is_i:ie_i,js_i:je_i,k))
                     call mpp_update_domains(q0(:,:,k), domain_i)
                  else
                     call MAPL_NCIOGetVarName(NCIO,lvar_cnt,vname)
                     call MAPL_VarRead(NCIO,vname,gslice_r4,lev=k)
...
! Horiz Interp for TR tracers
! is there a tr restart file to interpolate?
         if( file_exist("tr_internal_restart_in") .and. ntracers(6) > 0 ) then
            if (gid==0) print*, 'Trying to interpolate tr_internal_restart_in'

            call MAPL_NCIOGetFileType("tr_internal_restart_in",filetype)
            ...
            if (filetype /= 0) then
               ...
               lvar_cnt = 0 
               allocate(gslice_r4(im,jm))
               NCIO = MAPL_NCIOOpen("tr_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
               if (nVars /= iq_tr1-iq_tr0+1) call mpp_error(FATAL,'Wrong number of variables in tr file') 
               tileoff = (tile-1)*(jm/ntiles)
            end if

            do ivar=iq_tr0,iq_tr1
               if (filetype ==0) lvar_cnt=lvar_cnt+1
               do k=1,km
                  if (filetype /= 0) then
                     call parallel_read_file_r4('tr_internal_restart_in', npts, is_i,ie_i, js_i,je_i, 1, offset, q0(is_i:ie_i,js_i:je_i,k))
                     call mpp_update_domains(q0(:,:,k), domain_i)
                  else
                     call MAPL_NCIOGetVarName(NCIO,lvar_cnt,vname)
                     call MAPL_VarRead(NCIO,vname,gslice_r4,lev=k)
...
! Horz/Vert remap for scalars
         nqmap =  nmoist + ngocart + npchem + ngmichem + nstratchem + ntr
...
! Range check the MOIST tracers
      do iq=iq_moist0,iq_moist1
         ...
         enddo
      enddo
      do iq=1,nmoist+npchem+ngocart+ngmichem+nstratchem+ntr
   ...
   subroutine get_geos_latlon_ic( Atm, fv_domain, nq, ntracers)
      ...
      integer:: i1, i2, nmoist, ngocart, npchem, ngmichem, nstratchem, ntr, nqmap, offset
...
! is there a gmichem restart file to interpolate?
         if( file_exist("gmichem_internal_restart_in") .and. ntracers(4) > 0 ) then
            ...
            if (filetype /= 0) then
               open(IUNIT,file="gmichem_internal_restart_in" ,access='sequential',form='unformatted',status='old')
            else
               lvar_cnt = 0
               NCIO = MAPL_NCIOOpen("gmichem_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
            ...
            do ivar=iq_gmichem0,iq_gmichem1
               ...
               enddo
               if (gid==0) call pmaxmin( 'GMICHEM_Q_',  q0(:,:,:), im*jm, km, 1.d0)
            enddo
...
! is there a stratchem restart file to interpolate?
         if( file_exist("stratchem_internal_restart_in") .and. ntracers(5) > 0 ) then
            ...
            if (filetype /= 0) then
               open(IUNIT,file="stratchem_internal_restart_in" ,access='sequential',form='unformatted',status='old')
            else
               lvar_cnt = 0
               NCIO = MAPL_NCIOOpen("stratchem_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
            ...
            do ivar=iq_stratchem0,iq_stratchem1
               ...
               enddo
               if (gid==0) call pmaxmin( 'STRATCHEM_Q_',  q0(:,:,:), im*jm, km, 1.d0)
            enddo
...
! is there a tr restart file to interpolate?
         if( file_exist("tr_internal_restart_in") .and. ntracers(6) > 0 ) then
            ...
            if (filetype /= 0) then
               open(IUNIT,file="tr_internal_restart_in" ,access='sequential',form='unformatted',status='old')
            else
               lvar_cnt = 0
               NCIO = MAPL_NCIOOpen("tr_internal_restart_in")
               call MAPL_NCIOGetDimSizes(NCIO,nVars=nVars)
            ...
            do ivar=iq_tr0,iq_tr1
               ...
               enddo
               if (gid==0) call pmaxmin( 'TR_Q_',  q0(:,:,:), im*jm, km, 1.d0)
            enddo
...
! Horz/Vert remap for MOIST, GOCART, PCHEM, GMICHEM, STRATCHEM, and TR scalars (Assuming Total Number is divisible by KM)
! -----------------------------------------------------------------------------------------------------------------------
         nqmap = nmoist + ngocart + npchem + ngmichem + nstratchem + ntr
...
! Range check the MOIST tracers
      ...
      enddo
      if (gid==0) print*
      do iq=iq_gmichem0,iq_gmichem1
         call prt_maxmin('QP_GMICHEM_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1., gid==0)
      enddo 
      if (gid==0) print*
      do iq=iq_stratchem0,iq_stratchem1
         call prt_maxmin('QP_STRATCHEM_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1., gid==0)
      enddo 
      if (gid==0) print*
      do iq=iq_tr0,iq_tr1
         call prt_maxmin('QP_TR_Q', Atm(1)%q(is:ie,js:je,1:npz,iq), is, ie, js, je, 0, npz, 1., gid==0)
      enddo
