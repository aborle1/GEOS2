...
module fv_control_mod
...
#else
   use MAPL_MOD,           only: MAPL_KAPPA, MAPL_PI_R8, MAPL_RADIUS
#endif
   ...
   use fv_arrays_mod,      only: fv_atmos_type, REAL8, tiny_number, huge_number
   use fv_grid_utils_mod,  only: grid_utils_init, grid_utils_end, ptop_min, deglat, &
                                 da_min_c, da_min, cosa, sina
   use fv_grid_tools_mod,  only: init_grid, area, area_c, dx, dy, dxa, dya, &
...
! Tracer transport options:
   ...
   real(REAL8)    :: t_fac = 0.2    ! Divergence filtering (apply to pp in time), range [0,0.25]

   integer :: nord=3         ! 0: del-2, 1: del-4, 2: del-6, 3: del-8 divergence damping
                             ! Alternative setting for high-res: nord=1; d4_bg = 0.075
   real(REAL8)    :: dddmp = 0.0    ! coefficient for del-2 divergence damping (0.2)
                             ! for C90 or lower: 0.2
   real(REAL8)    :: d2_bg = 0.0    ! coefficient for background del-2 divergence damping
   real(REAL8)    :: d4_bg = 0.16   ! coefficient for background del-4(6) divergence damping
                             ! for stability, d4_bg must be <=0.16 if nord=3
   real(REAL8)    :: vtdm4 = 0.0    ! coefficient for del-4 vorticity damping
   real(REAL8)    :: d2_bg_k1 = 4.         ! factor for d2_bg (k=1)
   real(REAL8)    :: d2_bg_k2 = 2.         ! factor for d2_bg (k=2)
   real(REAL8)    :: d2_divg_max_k1 = 0.15 ! d2_divg max value (k=1)
   real(REAL8)    :: d2_divg_max_k2 = 0.08 ! d2_divg max value (k=2)
   real(REAL8)    :: damp_k_k1 = 0.2       ! damp_k value (k=1)
   real(REAL8)    :: damp_k_k2 = 0.12      ! damp_k value (k=2)
...
! PG off centering:
   real(REAL8)    :: beta  = 0.0    ! 0.5 is "neutral" but it may not be stable
   integer :: n_sponge = 1   ! Number of sponge layers at the top of the atmosphere
   real(REAL8)    :: d_ext = 0.02   ! External model damping (was 0.02)
...
!-----------------------------------------------------------
   real(REAL8) :: shift_fac   =  18.   ! shift west by 180/shift_fac = 10 degrees
! Defaults for Schmidt transformation:
   logical :: do_schmidt = .false. 
   real(REAL8) :: stretch_fac =   1.   ! No stretching
   real(REAL8) :: target_lat  = -90.   ! -90: no grid rotation 
   real(REAL8) :: target_lon  =   0.   ! 
...
#ifdef MARS_GCM
   real(REAL8)    :: p_ref = 600.
   real(REAL8)    :: reference_sfc_pres = 7.7E2
   real(REAL8)    :: sponge_damp=   1.0
   real(REAL8)    :: dry_mass = 7.7E2
#else
   real(REAL8)    :: p_ref = 1.E5
   real(REAL8)    :: dry_mass = 98290.
#endif
   integer :: nt_prog = 0
   integer :: nt_phys = 0
   real(REAL8)    :: tau_h2o = 0.            ! Time scale (days) for ch4_chem

   real(REAL8)    :: d_con = 0.
   real(REAL8)    :: consv_te = 0.
   real(REAL8)    :: tau = 0.                ! Time scale (days) for Rayleigh friction
   real(REAL8)    :: rf_center = 0.          ! Center position of the hyper-tan profile
                                      ! 0: use the top layer center
                                      ! > 0, [Pascal]

! When inlining QV we can do condensation to control super-saturation
   real(REAL8) :: SuperSat=1.025
   ...
   logical :: gmao_cubic = .true.     ! method for vertical remap of total energy, temperature or PT
                                      !  F           : GFDL schemes
                                      !  T (default) : GMAO cubic
   logical :: master
...
!------------------------------------------------
   ...
   real(REAL8)    :: dyn_timer, comm_timer
   ...
   public :: nord, fill, fill_dp, fill_wz, inline_q, breed_vortex_inline, dwind_2d, filter_phys, tq_filter 
...
!
      ...
      if (npz == 1) then
         ...
         n_sponge = -1           ! No Sponge Layers for Shallow Water
         nwat  = 0               ! No water species
         warm_start = .false.    !
         d_ext = 0
      endif
    ...
    ! Initialize the SW (2D) part of the model
      call grid_utils_init(Atm(1), Atm(1)%npx, Atm(1)%npy, Atm(1)%npz, Atm(1)%grid, Atm(1)%agrid,   &
                           area, area_c, dx, dy, dxa, dya, dxc, dyc, non_ortho,   &
      ...
      ! Allocate State Variables
        ...
        Atm(n)%u    = tiny_number
        Atm(n)%v    = tiny_number
        Atm(n)%pt   = tiny_number
        Atm(n)%delp = tiny_number
        Atm(n)%q    = tiny_number
      ...
      ! Allocate Auxilliary pressure arrays
        ...
        Atm(n)%ps   = tiny_number
        Atm(n)%pe   = tiny_number
        Atm(n)%pk   = tiny_number
        Atm(n)%peln = tiny_number
        Atm(n)%pkz  = tiny_number
      ...
      ! For tracer transport:
        ...
        Atm(n)%ts   = tiny_number
        Atm(n)%phis = tiny_number
        Atm(n)%ts   = tiny_number
        Atm(n)%omga = tiny_number
        Atm(n)%ua   = tiny_number
        Atm(n)%va   = tiny_number
        Atm(n)%uc   = tiny_number
        Atm(n)%vc   = tiny_number
        Atm(n)%mfx  = tiny_number
        Atm(n)%mfy  = tiny_number
        Atm(n)%cx   = tiny_number
        Atm(n)%cy   = tiny_number
...
!
      ...
      real(REAL8) :: dim0 = 180.           ! base dimension
      real(REAL8) :: dt0  = 1800.          ! base time step
      real(REAL8) :: ns0  = 5.             ! base nsplit for base dimension 
                                    ! For cubed sphere 5 is better
      real(REAL8) :: umax = 350.           ! max wave speed for grid_type>3
      real(REAL8) :: dimx, dl, dp, dxmin, dymin, d_fac
      ...
      namelist /fv_core_nml/npx, npy, ntiles, npz, npz_rst, layout, io_layout, ncnst, nwat,  &
                            ...
                            na_init, hybrid_z, Make_NH, m_grad_p, n_zs_filter, nord_zs_filter, &
                            reset_beta, a2b_ord, uniform_ppm, gmao_cubic, remap_t, k_top, m_riem, p_ref, d2_bg_k1, d2_bg_k2,  &
#ifdef MARS_GCM
...
!      vtdm4 = vtdm4 * d_fac
      ...
      if(master) then
         write(6,199) 'Using grid_type : ', grid_type
