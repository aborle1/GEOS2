...
#include "mpif.h"
   ...
   integer im,jm,km,nlon,nlat,nq,ntracers(6)
   ...
   integer :: nmoist,ngocart,npchem,ngmichem,nstratchem,ntr
   logical :: isBinFV, isBinMoist, isBinGocart, isBinPchem, isBinGmichem, isBinStratchem, isBinTr
   integer :: ftype
...
if( file_exist("gmichem_internal_restart_in") .and. (gid==masterproc) ) then
      call MAPL_NCIOGetFileType("gmichem_internal_restart_in",ftype)
      if (ftype == 0) then
         isBinGmichem = .false.
         NCIO = MAPL_NCIOOpen("gmichem_internal_restart_in")
         call MAPL_NCIOGetDimSizes(NCIO,slices=ngmichem)
         call MAPL_NCIOClose(NCIO,destroy=.true.)
      else
         call rs_count( "gmichem_internal_restart_in",ngmichem )
      end if
   endif
   call print_memuse_stats('interp_restarts: rs_count - ngmichem')
   call mpp_broadcast(ngmichem, masterproc)
   call mpp_broadcast(isBinGmichem, masterproc) 

if( file_exist("stratchem_internal_restart_in") .and. (gid==masterproc) ) then
      call MAPL_NCIOGetFileType("stratchem_internal_restart_in",ftype)
      if (ftype == 0) then
         isBinStratchem = .false.
         NCIO = MAPL_NCIOOpen("stratchem_internal_restart_in")
         call MAPL_NCIOGetDimSizes(NCIO,slices=nstratchem)
         call MAPL_NCIOClose(NCIO,destroy=.true.)
      else
         call rs_count( "stratchem_internal_restart_in",nstratchem )
      end if
   endif
   call print_memuse_stats('interp_restarts: rs_count - nstratchem')
   call mpp_broadcast(nstratchem, masterproc)
   call mpp_broadcast(isBinStratchem, masterproc) 

if( file_exist("tr_internal_restart_in") .and. (gid==masterproc) ) then
      call MAPL_NCIOGetFileType("tr_internal_restart_in",ftype)
      if (ftype == 0) then
         isBinTr = .false.
         NCIO = MAPL_NCIOOpen("tr_internal_restart_in")
         call MAPL_NCIOGetDimSizes(NCIO,slices=ntr)
         call MAPL_NCIOClose(NCIO,destroy=.true.)
      else
         call rs_count( "tr_internal_restart_in",ntr )
      end if
   endif
   call print_memuse_stats('interp_restarts: rs_count - ntr')
   call mpp_broadcast(ntr, masterproc)
   call mpp_broadcast(isBinTr, masterproc)
...
! Headers
      ...
      ntracers(4) = ngmichem/km
      ntracers(5) = nstratchem/km
      ntracers(6) = ntr/km
      nq = nmoist + ngocart + npchem + ngmichem  + nstratchem  + ntr
      iq_moist0 =           1
...
103      format(2x,a,i6,3x,a,f7.2,a)
         ...
         write(6,103) 'Total Number of Tracers in GMICHEM: ',ngmichem,'(/KM = ',float(ngmichem)/float(km),')'
         write(6,103) 'Total Number of Tracers in STRATCHEM: ',nstratchem,'(/KM = ',float(nstratchem)/float(km),')'
         write(6,103) 'Total Number of Tracers in TR: ',ntr,'(/KM = ',float(ntr)/float(km),')'
         print *
      endif
!endif
      close(IUNIT)
...
! write gmichem_internal_rst
      if( file_exist("gmichem_internal_restart_in") ) then
         write(fname1, "('gmichem_internal_rst_c',i4.4,'_',i3.3,'L')") npx-1,npz
         if (gid==0) print*, 'Writing with MPIIO: ', TRIM(fname1)
         call MPI_FILE_OPEN(MPI_COMM_WORLD, fname1, MPI_MODE_WRONLY+MPI_MODE_CREATE, MPI_INFO_NULL, MUNIT, STATUS)
         gsizes(1) = (npx-1)
         gsizes(2) = (npy-1) * 6
...
! Write Variables
         ...
         do iq=1,ngmichem/npz
            do k=1,npz
               varo_r4 = Atm(1)%q(is:ie,js:je,k,(nmoist+ngocart+npchem)/npz)
               call MPI_FILE_SET_VIEW(MUNIT, offset, MPI_REAL, filetype, "native", MPI_INFO_NULL, STATUS)
               call MPI_FILE_WRITE_ALL(MUNIT, varo_r4, lsize, MPI_REAL, mpistatus, STATUS)
               offset = offset + slice_2d*4 + 8
            enddo
...
! write stratchem_internal_rst
      if( file_exist("stratchem_internal_restart_in") ) then
         write(fname1, "('stratchem_internal_rst_c',i4.4,'_',i3.3,'L')") npx-1,npz
         if (gid==0) print*, 'Writing with MPIIO: ', TRIM(fname1)
         call MPI_FILE_OPEN(MPI_COMM_WORLD, fname1, MPI_MODE_WRONLY+MPI_MODE_CREATE, MPI_INFO_NULL, MUNIT, STATUS)
         gsizes(1) = (npx-1)
         gsizes(2) = (npy-1) * 6
...
! Write Variables
         ...
         do iq=1,nstratchem/npz
            do k=1,npz
               varo_r4 = Atm(1)%q(is:ie,js:je,k,(nmoist+ngocart+npchem+ngmichem)/npz)
               call MPI_FILE_SET_VIEW(MUNIT, offset, MPI_REAL, filetype, "native", MPI_INFO_NULL, STATUS)
               call MPI_FILE_WRITE_ALL(MUNIT, varo_r4, lsize, MPI_REAL, mpistatus, STATUS)
               offset = offset + slice_2d*4 + 8
            enddo
...
! write tr_internal_rst
      if( file_exist("tr_internal_restart_in") ) then
         write(fname1, "('tr_internal_rst_c',i4.4,'_',i3.3,'L')") npx-1,npz
         if (gid==0) print*, 'Writing with MPIIO: ', TRIM(fname1)
         call MPI_FILE_OPEN(MPI_COMM_WORLD, fname1, MPI_MODE_WRONLY+MPI_MODE_CREATE, MPI_INFO_NULL, MUNIT, STATUS)
         gsizes(1) = (npx-1)
         gsizes(2) = (npy-1) * 6
...
! Write Variables
         ...
         do iq=1,ntr/npz
            do k=1,npz
               varo_r4 = Atm(1)%q(is:ie,js:je,k,(nmoist+ngocart+npchem+ngmichem+nstratchem)/npz)
               call MPI_FILE_SET_VIEW(MUNIT, offset, MPI_REAL, filetype, "native", MPI_INFO_NULL, STATUS)
               call MPI_FILE_WRITE_ALL(MUNIT, varo_r4, lsize, MPI_REAL, mpistatus, STATUS)
               offset = offset + slice_2d*4 + 8
            enddo
...
!
      if( file_exist("gmichem_internal_restart_in") ) then
         write(fname1, "('gmichem_internal_rst_c',i4.4,'_',i3.3,'L')") npx-1,npz
         if (gid==0) print*, 'Writing : ', TRIM(fname1)
         if (isBingmichem) then
            if (gid==0) open(OUNIT,file=fname1,access='sequential',form='unformatted')
         else
            lvar=0
            if (gid==0) then
               ncio = MAPL_NCIOOpen("gmichem_internal_restart_in",rc=status)
               imc = npx-1
               jmc = imc*6
               call MAPL_NCIOChangeRes(ncio,ncioOut,lonSize=imc,latSize=jmc,levSize=npz)
               call MAPL_NCIOSet(ncioOut,filename=fname1)
         ...
         do iq=iq_gmichem0,iq_gmichem1
            if (gid==0) print*, 'Writing : ', TRIM(fname1), ' ', iq
            if (.not.isBinGmichem .and. gid==0) then
               lvar=lvar+1
               call MAPL_NCIOGetVarName(ncio,lvar,vname) 
            end if
            do k=1,npz
...
!
      if( file_exist("stratchem_internal_restart_in") ) then
         write(fname1, "('stratchem_internal_rst_c',i4.4,'_',i3.3,'L')") npx-1,npz
         if (gid==0) print*, 'Writing : ', TRIM(fname1)
         if (isBinstratchem) then
            if (gid==0) open(OUNIT,file=fname1,access='sequential',form='unformatted')
         else
            lvar=0
            if (gid==0) then
               ncio = MAPL_NCIOOpen("stratchem_internal_restart_in",rc=status)
               imc = npx-1
               jmc = imc*6
               call MAPL_NCIOChangeRes(ncio,ncioOut,lonSize=imc,latSize=jmc,levSize=npz)
               call MAPL_NCIOSet(ncioOut,filename=fname1)
         ...
         do iq=iq_stratchem0,iq_stratchem1
            if (gid==0) print*, 'Writing : ', TRIM(fname1), ' ', iq
            if (.not.isBinStratchem .and. gid==0) then
               lvar=lvar+1
               call MAPL_NCIOGetVarName(ncio,lvar,vname) 
            end if
            do k=1,npz
...
!
      if( file_exist("tr_internal_restart_in") ) then
         ...
         if (isBintr) then
            ...
            lvar=0
            if (gid==0) then
               ncio = MAPL_NCIOOpen("tr_internal_restart_in",rc=status)
               imc = npx-1
               jmc = imc*6
               call MAPL_NCIOChangeRes(ncio,ncioOut,lonSize=imc,latSize=jmc,levSize=npz)
               call MAPL_NCIOSet(ncioOut,filename=fname1)
