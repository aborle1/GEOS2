...
! Local derived type aliases
    ...
    real, pointer                           :: maskptr(:) => NULL()
    real, allocatable, dimension(:)         :: globalmask, dischargemasktile
    real, allocatable, dimension(:)         :: local_mask
...
!-----------------------------------
    ...
    if (RoutingFile /= "") then
       ...
       call MAPL_LocStreamGet(LocStream, NT_LOCAL=NT, RC=STATUS )
       VERIFY_(STATUS)
       allocate(globalmask(NT), dischargemasktile(NT), stat=status)
       VERIFY_(STATUS)
...
!          find nt_local
          ...
          call MAPL_Get (CHILD_MAPL, LOCSTREAM=CHILD_LS, RC=STATUS )
          VERIFY_(STATUS)
          call MAPL_LocStreamGet(CHILD_LS, NT_LOCAL=NT_LOCAL, RC=STATUS )
          VERIFY_(STATUS)
          if (nt_local == 0) cycle
          allocate (local_mask(nt_local), stat=status)
          VERIFY_(STATUS)
          local_mask = 1.0
          call MAPL_LocStreamTransform( globalmask, &
               surf_internal_state%xform_out(I), local_mask, RC=STATUS ) 
          VERIFY_(STATUS)
          deallocate (local_mask)
       END DO
       call RouteRunoff(SURF_INTERNAL_STATE%LocalRoutings, &
                        globalmask, dischargemasktile, RC=STATUS)
       VERIFY_(STATUS)

       call MAPL_GetPointer(GIM(OCEAN), maskptr, 'DISCHARGE_MASK')
       if (associated(maskptr))THEN
!       VERIFY_(STATUS)
       ! pack dischargemasktile with MASK=OCEAN
       K = 0
       DO I = 1, NT
          if (tiletype(I) == MAPL_OCEAN) then
             K = K+1
             maskptr(K) = dischargemasktile(I)
          end if
       END DO
       ENDIF
       deallocate(globalmask, dischargemasktile)

       call MAPL_GetObjectFromGC ( GCS(OCEAN), CHILD_MAPL, RC=STATUS )
       VERIFY_(STATUS)
       call MAPL_Get (CHILD_MAPL, NumInitPhases=NumInitPhases, RC=STATUS )
       VERIFY_(STATUS)

       if (NumInitPhases > 1) THEN
          call ESMF_GridCompInitialize(GCS(OCEAN), &
               importState=GIM(OCEAN), exportState=GEX(OCEAN), &
               clock=CLOCK, PHASE=2, userRC=userRC, RC=STATUS )
...
! Local derived type aliases
    ...
    integer                             :: NUM_AERO_DP
    integer, parameter                  :: NUM_DUDP_MAX = 5, NUM_DUWT_MAX = 5, NUM_DUSD_MAX = 5
    integer, parameter                  :: NUM_BCDP_MAX = 2, NUM_BCWT_MAX = 2
    integer, parameter                  :: NUM_OCDP_MAX = 2, NUM_OCWT_MAX = 2
    integer                             :: NUM_DUDP, NUM_DUWT, NUM_DUSD
    integer                             :: NUM_BCDP, NUM_BCWT
    integer                             :: NUM_OCDP, NUM_OCWT
...
! Pointers to exports
    ...
    real, pointer, dimension(:,:) :: DISCHARGEIN  => NULL()
    real, pointer, dimension(:,:) :: DISCHARGEOUT => NULL()
...
! These are tile versions of exports
    ...
    real, pointer, dimension(:) :: DISCHARGETILEIN  => NULL()
    real, pointer, dimension(:) :: DISCHARGETILEOUT => NULL()
...
!------------------------------------------------------
    ...
    ! Sort AERO_DP into individual constituents
    NUM_DUDP = 0 ; NUM_DUWT = 0 ; NUM_DUSD = 0
    NUM_BCDP = 0 ; NUM_BCWT = 0
    NUM_OCDP = 0 ; NUM_OCWT = 0

    do K = 1, NUM_AERO_DP
       FIELD_NAME = trim(adjustl(AERO_DP_FIELD_NAME(K)))
       if ( FIELD_NAME(1:4) == 'DUDP' ) NUM_DUDP = NUM_DUDP + 1
       if ( FIELD_NAME(1:4) == 'DUWT' ) NUM_DUWT = NUM_DUWT + 1
       if ( FIELD_NAME(1:4) == 'DUSD' ) NUM_DUSD = NUM_DUSD + 1
       if ( FIELD_NAME(1:4) == 'BCDP' ) NUM_BCDP = NUM_BCDP + 1
       if ( FIELD_NAME(1:4) == 'BCWT' ) NUM_BCWT = NUM_BCWT + 1
       if ( FIELD_NAME(1:4) == 'OCDP' ) NUM_OCDP = NUM_OCDP + 1
       if ( FIELD_NAME(1:4) == 'OCWT' ) NUM_OCWT = NUM_OCWT + 1
    end do

    K = 1
    if ( NUM_DUDP /= 0 ) then
        allocate( DUDP(IM,JM,NUM_DUDP_MAX), STAT=STATUS ); VERIFY_(STATUS)
        DUDP(:,:,1:NUM_DUDP) = AERO_DP(:,:,K:NUM_DUDP)
    endif

    K = K + NUM_DUDP
    if ( NUM_DUWT /= 0 ) then
        allocate( DUWT(IM,JM,NUM_DUWT_MAX), STAT=STATUS ); VERIFY_(STATUS)
        DUWT(:,:,1:NUM_DUWT) = AERO_DP(:,:,K:NUM_DUWT)
    endif

    K = K + NUM_DUWT
    if ( NUM_DUSD /= 0 ) then
        allocate( DUSD(IM,JM,NUM_DUSD_MAX), STAT=STATUS ); VERIFY_(STATUS)
        DUSD(:,:,1:NUM_DUSD) = AERO_DP(:,:,K:NUM_DUSD)
    endif

    K = K + NUM_DUSD
    if ( NUM_BCDP /= 0 ) then
        allocate( BCDP(IM,JM,NUM_BCDP_MAX), STAT=STATUS ); VERIFY_(STATUS)
        BCDP(:,:,1:NUM_BCDP) = AERO_DP(:,:,K:NUM_BCDP)
    endif

    K = K + NUM_BCDP
    if ( NUM_BCWT /= 0 ) then
        allocate( BCWT(IM,JM,NUM_BCWT_MAX), STAT=STATUS ); VERIFY_(STATUS)
        BCWT(:,:,1:NUM_BCWT) = AERO_DP(:,:,K:NUM_BCWT)
    endif

    K = K + NUM_BCWT
    if ( NUM_OCDP /= 0 ) then
        allocate( OCDP(IM,JM,NUM_OCDP_MAX), STAT=STATUS ); VERIFY_(STATUS)
        OCDP(:,:,1:NUM_OCDP) = AERO_DP(:,:,K:NUM_OCDP)
    endif

    K = K + NUM_OCDP
    if ( NUM_OCWT /= 0 ) then
        allocate( OCWT(IM,JM,NUM_OCWT_MAX), STAT=STATUS ); VERIFY_(STATUS)
        OCWT(:,:,1:NUM_OCWT) = AERO_DP(:,:,K:NUM_OCWT)
    endif
...
! if we are running Louis sfc layer, get these exports from the gridded Louis fluxes
  if (CHOOSEMOSFC.eq.0) then
    ...
    call MAPL_GetPointer(EXPORT  , DISCHARGEIN,'DISCHARGE_IN',RC=STATUS); VERIFY_(STATUS)
    call MAPL_GetPointer(EXPORT  , DISCHARGEOUT,'DISCHARGE_OUT',RC=STATUS); VERIFY_(STATUS)
...
! RiverRouting: force allocations and make sure RoutingFile was specified
    if (associated(DISCHARGE)) then
       ASSERT_(associated(SURF_INTERNAL_STATE%LocalRoutings))
       call MAPL_GetPointer(EXPORT, RUNOFF, 'RUNOFF', alloc=.true.,  RC=STATUS)
       VERIFY_(STATUS)
    end if
...
!--------------------------------------------------------
    ...
    call MKTILE(RUNOFF  ,RUNOFFTILE  ,NT,RC=STATUS); VERIFY_(STATUS)
    call MKTILE(DISCHARGE,DISCHARGETILE,NT,RC=STATUS); VERIFY_(STATUS)
    call MKTILE(DISCHARGEIN,DISCHARGETILEIN,NT,RC=STATUS); VERIFY_(STATUS)
    call MKTILE(DISCHARGEOUT,DISCHARGETILEOUT,NT,RC=STATUS); VERIFY_(STATUS)
    ...
    if (associated(DISCHARGETILE)) DISCHARGETILE = 0.0
    if (associated(DISCHARGETILEIN)) DISCHARGETILEIN = 0.0
    if (associated(DISCHARGETILEOUT)) DISCHARGETILEOUT = 0.0

    do I = 1, NUM_CHILDREN
       ! to collect RUNOFF we need the handle OCEAN last and separately
       if (I == OCEAN) cycle
    ...
    if(associated(  RUNOFF)) then
       call MAPL_LocStreamTransform( LOCSTREAM, RUNOFF, &
            RUNOFFTILE, RC=STATUS) 
       VERIFY_(STATUS)
    endif
    if(associated(DISCHARGE))then
       call RouteRunoff(SURF_INTERNAL_STATE%LocalRoutings, &
            RUNOFFTILE, DISCHARGETILE, RC=STATUS)
       VERIFY_(STATUS)

       call MAPL_LocStreamTransform( LOCSTREAM, DISCHARGE, &
            DISCHARGETILE, RC=STATUS) 
       VERIFY_(STATUS)

       call MAPL_GetPointer(GIM(OCEAN), dataptr, 'DISCHARGE')
       if (associated(dataptr))then
          !       VERIFY_(STATUS)
          ! pack dischargetile with MASK=OCEAN
          K = 0
          DO N = 1,NT
             if (tiletype(N) == MAPL_OCEAN) then
                K = K+1
                dataptr(K) = dischargetile(N)
             end if
          END DO
       endif
    endif
...
!****************************************************************************
    ...
    if(associated(DISCHARGEIN)) then
       call MAPL_GetPointer(GIM(OCEAN), dataptr, 'DISCHARGE', rc=status)
       VERIFY_(STATUS)
       ! unpack dischargetile with MASK=OCEAN
       K = 0
       DO N = 1,NT
          if (tiletype(N) == MAPL_OCEAN) then
             K = K+1
             dischargetilein(N) = dataptr(K)
          end if
       END DO

       call MAPL_LocStreamTransform( LOCSTREAM, DISCHARGEIN, &
            DISCHARGETILEIN, RC=STATUS) 
       VERIFY_(STATUS)
    end if
    if(associated(DISCHARGEOUT)) then
       call MAPL_GetPointer(GEX(OCEAN), dataptr, 'DISCHARGE', rc=status)
       VERIFY_(STATUS)
       ! unpack dischargetile with MASK=OCEAN
       K = 0
       DO N = 1,NT
          if (tiletype(N) == MAPL_OCEAN) then
             K = K+1
             dischargetileout(N) = dataptr(K)
          end if
       END DO
       call MAPL_LocStreamTransform( LOCSTREAM, DISCHARGEOUT, &
            DISCHARGETILEOUT, RC=STATUS) 
       VERIFY_(STATUS)
    end if
...
!---------
    ...
    if(associated(DISCHARGETILEIN))deallocate(DISCHARGETILEIN)
    if(associated(DISCHARGETILEOUT))deallocate(DISCHARGETILEOUT)
...
!-------------------------------------------------------------
  ...
  subroutine MKTILE(VAR, TILEVAR, NT, RC)
    real, pointer                  :: VAR(:,:)
    real, pointer                  :: TILEVAR(:)
    integer,           intent(IN)  :: NT
    integer, optional, intent(OUT) :: RC

    character(len=ESMF_MAXSTR)   :: IAm='MKTILE'
    integer                      :: STATUS
...
!-----------------------------------------------------
  ...
  subroutine FILLOUT_TILE(STATE, NAME, TILE, XFORM, RC)
    type(ESMF_STATE),   intent(INOUT) :: STATE
...
!  Locals
    ...
    character(len=ESMF_MAXSTR)   :: IAm='FILLOUT_TILE'
    integer                      :: STATUS
    real, pointer                :: PTR(:)
    type (ESMF_Field)            :: field
...
!--------------------------------------------------------------      
  ...
  end subroutine FILLOUT_TILE

    subroutine RouteRunoff(Routing, Runoff, Discharge, Drainage, rc)
      type(T_Routing),  intent(IN   ) :: Routing(:)
      real,             intent(IN   ) :: Runoff(:)
      real,             intent(INOUT) :: Discharge(:)
      real,    optional,intent(INOUT) :: Drainage(:)
      integer, optional,intent(  OUT) :: rc
      ...
      Discharge = 0.0
      addDrainage = .false.
      if (present(Drainage)) addDrainage = .true.
      ...
      do i=1,size(Routing)
         if(Routing(i)%SrcPE==myPE) then
            ...
            if(Routing(i)%DstPE==myPE) then
               if(Routing(i)%DstIndex==Routing(i)%SrcIndex) then
                  if (addDrainage) then
                     Drainage(Routing(i)%DstIndex) = Drainage(Routing(i)%DstIndex) + TileDischarge
                  endif
               else
