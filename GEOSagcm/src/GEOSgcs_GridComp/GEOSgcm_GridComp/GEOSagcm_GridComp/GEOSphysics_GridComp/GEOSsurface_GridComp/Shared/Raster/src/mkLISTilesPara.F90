...
SUBROUTINE create_files_esa (nc, nr, gfile,filename)
 ...
 !-------------------------------
  ...
  regrid = .true.
  dx_esa = nc_esa / nc_global ! x-dimension (or # of ESA columns within the raster grid cell)
  dy_esa = nr_esa / nr_global ! y-dimension (or # of ESA rows within the raster grid cell)
  msk2rst = nc_esa/nc
  ...
  do catNo = 1, ncells 
     ...
     if (counted) then
        status  = NF_GET_VARA_INT (ncid,4,(/(ix1-1)*msk2rst+1,(iy1-1)*msk2rst +1/), &
             (/(ix2-ix1+1)*msk2rst,dy_esa/),geos_msk) ! Read 10-arcsec rows that lie within the raster row 'j' 
        if(status /=0) then
           PRINT *, NF_STRERROR(STATUS)
