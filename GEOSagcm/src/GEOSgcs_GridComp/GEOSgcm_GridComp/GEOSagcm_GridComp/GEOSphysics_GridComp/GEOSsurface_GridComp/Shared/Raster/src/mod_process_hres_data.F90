...
public :: soil_para_hwsd,hres_lai,hres_gswp2, merge_lai_data
public :: modis_alb_on_tiles_high,modis_scale_para_high,hres_lai_no_gswp
public :: histogram, regrid_map, create_mapping, esa2mosaic , esa2clm
...
! --------------------------------
    ...
    do k = 1, maxcat
       ...
       if(sum(tem).gt.0)then
          ...
          write (10,'(i8,i8,2(2x,i3),2(2x,f6.2))')     &
            tid,cid,mos1,mos2,100.*mfrac,100.*sfrac
...
!
        ...
        SUBROUTINE CREATE_ROUT_PARA_FILE (NC, NR, gfile, MGRID, deltaXY)
          ...
          if (PRESENT (deltaXY)) then
             if (deltaXY <  0.125) max_pfaf_smap = 8
