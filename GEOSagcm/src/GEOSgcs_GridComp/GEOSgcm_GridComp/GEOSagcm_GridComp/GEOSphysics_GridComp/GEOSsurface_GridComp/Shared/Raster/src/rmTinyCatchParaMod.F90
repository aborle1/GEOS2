...
!       if(sum(tem).le.0.) stop
    ...
    do j=1,maxcat
       ...
       write (10,'(i8,i8,2(2x,i3),2(2x,f6.4))')     &
            j+ip1,id(j+ip1),mos_veg(j-1,1),mos_veg(j-1,2),veg_frac(j,1),veg_frac(j,2)
            mos_veg(j-1,1) = mos_veg(j,1)
            mos_veg(j-1,2) = mos_veg(j,2)
       else
       write (10,'(i8,i8,2(2x,i3),2(2x,f6.4))')     &
            j+ip1,id(j+ip1),mos_veg(j,1),mos_veg(j,2),veg_frac(j,1),veg_frac(j,2)
       endif
    end do
    close(10,status='keep')
    deallocate (sib_veg2,sib_veg,mos_veg,veg_frac,zdep2_g,id)
    if(regrid) then
       deallocate(raster)
    endif     
