...
! pointers to import
   ...
   real,  dimension(NUM_CONSTIT)  :: TOTDEPOS
   real                           :: WSS
...
! -----------------
    ...
    VERIFY_(STATUS) 
    allocate(RCONSTIT(NUM_SNOW_LAYERS,1) , STAT=STATUS)
    VERIFY_(STATUS)  
...
! Get the zenith angle at the center of the time between the last solar call and the next one
    ...
    if(N==SNOW) then
       ...
       call    SNOW_ALBEDO(NT, NUM_SNOW_LAYERS, 0, ITYPE, LAI, ZTH, & 
...
!------------------
    ...
    deallocate(MLT , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(DTS , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(DQS , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(SHF , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(LHF , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(SHD , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(LHD , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(CFT , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(CFQ , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(SWN , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(DIF , STAT=STATUS)
    VERIFY_(STATUS)              
    deallocate(ULW , STAT=STATUS)
    VERIFY_(STATUS)                  

    deallocate(PRECIP  , STAT=STATUS)
    VERIFY_(STATUS)                  
    deallocate(RAIN    , STAT=STATUS)
    VERIFY_(STATUS)                  
    deallocate(RAINRF  , STAT=STATUS)
    VERIFY_(STATUS)                  
    deallocate(PERC    , STAT=STATUS)
    VERIFY_(STATUS)                  
    deallocate(MELTI   , STAT=STATUS)
    VERIFY_(STATUS)                  
    deallocate(FROZFRAC, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(TPSN    , STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(AREASC  , STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(HCORR   , STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(ghflxsno, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(ghflxice, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(HLWO    , STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(EVAPO   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(LHFO    , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(SHFO    , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(ZTH     , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(SLR     , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(EVAPI   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(DEVAPDT , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(ITYPE   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(LAI     , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(GRN     , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(MODISFAC, STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(SNOVR, SNONR, SNOVF, SNONF , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(LNDVR, LNDNR, LNDVF, LNDNF , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(VSUVR   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(VSUVF   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(SWNETSNOW, STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(RADDN   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(FHGND   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(DRHO0   , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(EXCS    , STAT=STATUS)
    VERIFY_(STATUS) 
    deallocate(WESNSC, SNDZSC, WESNPREC,    & 
             SNDZPREC,  SNDZ1PERC, WESNBOT, &
             STAT=STATUS) 
    VERIFY_(STATUS)
    deallocate(WESNPERC, WESNDENS, WESNREPAR, STAT=STATUS) 
    VERIFY_(STATUS)
    deallocate(LANDICELT, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(RCONSTIT, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(WESNN, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(HTSNN, STAT=STATUS)
    VERIFY_(STATUS)
    deallocate(SNDZN, STAT=STATUS)
    VERIFY_(STATUS)
