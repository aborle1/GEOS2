...
! ------------------------------
    ...
    real :: firefac ! gkw: fire tuning factor. 0: threshold WPWET; 1: threshold 1; <0: no fires
...
! Get the zenith angle at the center of the time between the last solar call and the next one
        ...
        call   SNOW_ALBEDO(NTILES,N_snow, 0, VEG1, LAI1, ZTH,        &
                 RHOFS,                                              &   
        ...
        call   SNOW_ALBEDO(NTILES,N_snow, 0, VEG2, LAI2, ZTH,        &
                 RHOFS,                                              &   
                 SNWALB_VISMAX, SNWALB_NIRMAX, SLOPE,                & 
                 WESNN, HTSNNN, SNDZN,                               &
...
! -------------------------------
    ...
    do nz = 1,nzone
      ...
      call compute_rc(ntiles,nveg,TCx,QAx,TA,PS,ZTH,DRPAR,DFPAR,            &
                      ...
                      dayl_fac,co2,dtc,dea,parzone,sifsunx,sifshax)
...
! --------------------------
    call MAPL_GetResource ( MAPL, DTCN, Label="DTCN:", DEFAULT=DT, RC=STATUS)
    VERIFY_(STATUS)
...
! check for fire tuning factor
! ----------------------------
    call MAPL_GetResource ( MAPL, firefac, Label="FIRE_FACTOR:", DEFAULT=-1., RC=STATUS) ! gkw: default is no fires
    VERIFY_(STATUS)
...
!   print *, 'CN_exit:',AGCM_YY,AGCM_MM,AGCM_DD,AGCM_S,dofyr
  endif
...
! -----------------------
        call CATCHCN ( NTILES, LONS, LATS                         ,&
	     ...
	     TSURF                                                ,&
             ...
             TCSORIG1, TPSN1IN1, TPSN1OUT1, LHACC=LHACC           )
