...
contains
  ...
  subroutine rfg2d_fft( &
       rseed, variance, N_x, N_y, dx, dy, lambda_x, lambda_y, &
       field1, field2 )
...
! random seed: do NOT modify the seed outside ran2() and gasdev()

! long int SEED_RAN2          = -999999;    ! use negative integer */

program test_rfg2d
  
  use prop_global_parameters
  use random_fields
  use nr_ran2_gasdev
  ...
  do n_e=1,N_e,2
     ...
     file_name = 'rf' // n_e_string(1:len_trim(n_e_string)) // '.dat'
     
     write(tmp_string, '(i3.3)') N_y
     
     output_format = '(' // tmp_string(1:len_trim(tmp_string)) // '(1x,e13.5))'

     open (10,file=file_name,status='new')
