...
!------------------------------------------------------------
    ...
    VEGDYN  = MAPL_AddChild(GC, NAME='VEGDYN', SS=VegdynSetServices, RC=STATUS)
    VERIFY_(STATUS)
...
! -------------------------------------------------------
    ...
    CASE (1) 
       ...
       VERIFY_(STATUS)
       if (NUM_CATCH == 1) then
          CATCH(1) = MAPL_AddChild(GC, NAME='CATCH', SS=CatchSetServices, RC=STATUS)
          VERIFY_(STATUS)
       else
          do I = 1, NUM_CATCH
             WRITE(TMP,'(I3.3)') I
    ...
    CASE (2) 
       ...
       VERIFY_(STATUS)
       if (NUM_CATCH == 1) then
          CATCHCN(1) = MAPL_AddChild(GC, NAME='CATCHCN', SS=CatchCNSetServices, RC=STATUS)
          VERIFY_(STATUS)
       else
          do I = 1, NUM_CATCH
             WRITE(TMP,'(I3.3)') I
    ...
    DO I = 1, NUM_CATCH
       ...
       CASE (1) 
          call MAPL_AddConnectivity (                                    &
            GC                                                 ,         &
            SHORT_NAME  = (/'LAI  ','GRN  ','ROOTL','Z2CH ', 'ITY  ' /) ,&
            DST_ID =  CATCH(I)                                 ,         &
            SRC_ID =  VEGDYN                                   ,         &
                                                      RC=STATUS )
          VERIFY_(STATUS)
