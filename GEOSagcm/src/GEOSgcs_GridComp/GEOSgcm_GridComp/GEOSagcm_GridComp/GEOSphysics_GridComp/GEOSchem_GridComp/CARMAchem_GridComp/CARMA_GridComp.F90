...
!  Black Carbon
   real, pointer, dimension(:,:) :: blackcarbon_emissions => null()

!  Smoke
   real, pointer, dimension(:,:) :: smoke_source => null()
   real, pointer, dimension(:,:,:) :: smoke_altitude => null()

!  Sulfate
   real, pointer :: so2biomass_src_(:,:) => null() ! before diurnal
   real, pointer :: so2biomass_src(:,:) => null()  ! after diurnal
   real, pointer :: so2anthro_l1_src(:,:) => null()  ! level 1
   real, pointer :: so2anthro_l2_src(:,:) => null()  ! level 2
   real, pointer :: so2ship_src(:,:) => null()
   real, pointer :: so4ship_src(:,:) => null()
   real, pointer :: aircraft_fuel_src(:,:,:) => null()
   real, pointer :: dmso_conc(:,:) => null()
   real, pointer :: vLat(:)    => null(), &
                    vLon(:)    => null(), &
                    vSO2(:)    => null(), &
                    vElev(:)   => null(), &
                    vCloud(:)  => null()
   real, pointer :: oh_conc(:,:,:) => null()
   real, pointer :: no3_mr(:,:,:) => null()
   real, pointer :: h2o2_mr(:,:,:) => null()
   real, pointer :: h2o2_int(:,:,:) => null()
...
!    Sulfate
     logical            :: GOCART_H2SO4    ! GOCART provides H2SO4
     logical            :: foundGOCARTtrop  = .false. ! tropospheric sulfate 
     logical            :: foundGOCARTvolc  = .false. ! volcanic sulfate
     logical            :: foundGOCARTstrat = .false. ! stratospheric sulfate
