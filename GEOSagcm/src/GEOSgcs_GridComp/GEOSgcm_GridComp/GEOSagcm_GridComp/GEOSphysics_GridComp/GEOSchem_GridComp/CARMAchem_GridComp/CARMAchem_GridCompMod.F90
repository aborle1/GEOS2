...
!   --------------------------
    ...
    logical                           :: foundGOCARTvolc, foundGOCARTtrop, foundGOCARTstrat
...
!  If we want sulfate production from GOCART then we will add the imports
   if(r%GOCART_H2SO4) then

     foundGOCARTtrop  = .false.
     foundGOCARTvolc  = .false.
     foundGOCARTstrat = .false.
     IF(state%chemReg%doing_SU) THEN
        
      DO n = state%chemReg%i_SU, state%chemReg%j_SU
       if( TRIM(state%chemReg%vname(n)) .eq. 'SO4v') then

        foundGOCARTvolc     = .true.
        r%foundGOCARTvolc   = .true.
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4Gvolc',  &
         LONG_NAME          = 'SO4 Prod from SO2 oxidation (Volcanic)',  &
         UNITS              = 'kg m-2 s-1', &
        ...
        VERIFY_(STATUS)
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4WETvolc',  &
         LONG_NAME          = 'SO4 Prod from wet SO2 oxidation (Volcanic)',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
        VERIFY_(STATUS)
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4AQvolc',  &
         LONG_NAME          = 'SO4 Prod from aqueous SO2 oxidation (Volcanic)',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
      ...
      END IF
       ...
       if( TRIM(state%chemReg%vname(n)) .eq. 'SO4s') then

        foundGOCARTstrat     = .true.
        r%foundGOCARTstrat   = .true.
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4Gstrat',  &
         LONG_NAME          = 'SO4 Prod from SO2 oxidation (Stratospheric)',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
        VERIFY_(STATUS)
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4WETstrat',  &
         LONG_NAME          = 'SO4 Prod from wet SO2 oxidation (Stratospheric)',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
        VERIFY_(STATUS)
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4AQstrat',  &
         LONG_NAME          = 'SO4 Prod from aqueous SO2 oxidation (Stratospheric)',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
      ...
      END IF
       ...
       if( TRIM(state%chemReg%vname(n)) .eq. 'SO4') then

        foundGOCARTtrop     = .true.
        r%foundGOCARTtrop   = .true.
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4G',  &
         LONG_NAME          = 'SO4 Prod from SO2 oxidation',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
        ...
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4WET',  &
         LONG_NAME          = 'SO4 Prod from wet SO2 oxidation',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
        ...
        CALL MAPL_AddImportSpec(GC,                                  &
         SHORT_NAME         = 'PSO4AQ',  &
         LONG_NAME          = 'SO4 Prod from aqueous SO2 oxidation',  &
         UNITS              = 'kg m-2 s-1', &
         DIMS               = MAPL_DimsHorzVert,    &
         VLOCATION          = MAPL_VLocationCenter,    &
                                                        RC=STATUS  )
    ...
    END IF
    if( (.not. foundGOCARTvolc) .and. (.not. foundGOCARTtrop ) .and. &
                                      (.not. foundGOCARTstrat) )  r%GOCART_H2SO4 = .false.
...
!        This is a placeholder code in case sometime I want CARMA to be "data_drive"
...
!  Destroy emissions
!  -----------------
   if(associated(gcCARMA%dust_source))       deallocate( gcCARMA%dust_source, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%blackcarbon_emissions)) deallocate( gcCARMA%blackcarbon_emissions, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%smoke_source))      deallocate( gcCARMA%smoke_source, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%smoke_altitude))    deallocate( gcCARMA%smoke_altitude, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so2biomass_src_))   deallocate( gcCARMA%so2biomass_src_, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so2biomass_src))    deallocate( gcCARMA%so2biomass_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so2anthro_l1_src))  deallocate( gcCARMA%so2anthro_l1_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so2anthro_l2_src))  deallocate( gcCARMA%so2anthro_l2_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so2ship_src))       deallocate( gcCARMA%so2ship_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%so4ship_src))       deallocate( gcCARMA%so4ship_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%aircraft_fuel_src)) deallocate( gcCARMA%aircraft_fuel_src, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%dmso_conc))         deallocate( gcCARMA%dmso_conc, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%vLat))              deallocate( gcCARMA%vLat, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%vLon))              deallocate( gcCARMA%vLon, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%vSO2))              deallocate( gcCARMA%vSO2, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%vElev))             deallocate( gcCARMA%vElev, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%vCloud))            deallocate( gcCARMA%vCloud, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%oh_conc))           deallocate( gcCARMA%oh_conc, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%no3_mr))            deallocate( gcCARMA%no3_mr, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%h2o2_mr))           deallocate( gcCARMA%h2o2_mr, stat=STATUS)
   VERIFY_(STATUS)
   if(associated(gcCARMA%h2o2_int))          deallocate( gcCARMA%h2o2_int, stat=STATUS)
   VERIFY_(STATUS)

!  Destroy Legacy state
!  --------------------
   call registry_destroy_ (state%CARMAreg)
   deallocate ( state%CARMAreg, state%qa, state%gcCARMA, state%chemReg, stat = STATUS )
   VERIFY_(STATUS)
...
!  Get sulfuric acid (H2SO4) from GOCART
!  ---------------
   CALL I90_label ( 'GOCART_H2SO4:', ier(1) )
   i = I90_gint( ier(2) )
   if ( any(ier(1:2) /= 0) ) then
      call final_(50)
      return
...
!   -----------------------------------------
    ...
    print *, 'GOCART_H2SO4:  ', r%gocart_h2so4
