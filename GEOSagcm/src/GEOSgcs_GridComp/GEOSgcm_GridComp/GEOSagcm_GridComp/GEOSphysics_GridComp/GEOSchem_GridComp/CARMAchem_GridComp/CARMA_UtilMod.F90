...
!  Utility Modules
   ...
   use SmokeEmissionMod      ! Smoke Emissions
...
!   real, parameter :: dM(8) = &      ! GOCART like PSD
...
!!  Dust 16-bin specific values
!   real, parameter :: dM(16) = &      ! d'Almeida initial PSD mass fractions
!    (/ 0.00110,  0.00217,  0.00367,  0.00555,  0.00824,  0.01379,  0.02663, &
!       0.05203,  0.09102,  0.13465,  0.16511,  0.16671,  0.13841,  0.09467, &
!       0.05374,  0.02573 /)

!  22-bin specific values
   real, parameter :: dM(22) = &   ! Kok particle size distribution
    (/ 0.0000,  0.0000,  0.0000,  0.0001,  0.0002,  0.0004,  0.0009,  0.0019, &
       0.0039,  0.0077,  0.0141,  0.0255,  0.0423,  0.0697,  0.1069,  0.1488, &
       0.1936,  0.1974,  0.1405,  0.0431,  0.0030,  0.0000 /)
...
!-------------------------------------------------------------------------
   ...
   real, pointer             :: radius_real(:), dr_real(:), rhop_real(:)
   real, pointer             :: w10m(:,:)
   ...
   REAL, POINTER, DIMENSION(:,:,:) :: pso4g_s, pso4wt_s, pso4aq_s
   REAL, POINTER, DIMENSION(:,:,:) :: pso4g_v, pso4wt_v, pso4aq_v
   REAL, POINTER, DIMENSION(:,:,:) :: pso4g_t, pso4wt_t, pso4aq_t
   REAL, POINTER, DIMENSION(:,:)   :: gwettop, fraclake, oro, u10m, v10m, &
                                      ustar, pblh, z0h, shflux, precc, precl, &
                                      frocean, frseaice, tskin, area
...
!   call MAPL_GetPointer ( impChem, tskin, 'TS', __RC__)
   ...
   if(reg%GOCART_H2SO4) then
    if(reg%foundGOCARTstrat) then
     call MAPL_GetPointer ( impChem, pso4g_s,  'PSO4Gstrat', __RC__)
     call MAPL_GetPointer ( impChem, pso4aq_s, 'PSO4AQstrat', __RC__)
     call MAPL_GetPointer ( impChem, pso4wt_s, 'PSO4WETstrat', __RC__)
    endif
    if(reg%foundGOCARTvolc) then
     call MAPL_GetPointer ( impChem, pso4g_v,  'PSO4Gvolc', __RC__)
     call MAPL_GetPointer ( impChem, pso4aq_v, 'PSO4AQvolc', __RC__)
     call MAPL_GetPointer ( impChem, pso4wt_v, 'PSO4WETvolc', __RC__)
    endif
    if(reg%foundGOCARTtrop) then
     call MAPL_GetPointer ( impChem, pso4g_t,  'PSO4G', __RC__)
     call MAPL_GetPointer ( impChem, pso4aq_t, 'PSO4AQ', __RC__)
     call MAPL_GetPointer ( impChem, pso4wt_t, 'PSO4WET', __RC__)
    endif
   endif
...
!    Read dust source function from file if not already allocated
     if( .not. associated(gcCARMA%dust_source)) then
      allocate(gcCARMA%dust_source(i1:i2,j1:j2),stat=STATUS)
      VERIFY_(STATUS)
      gcCARMA%dust_source = 0.

      call MAPL_GetPointer( impChem, gcCARMA%dust_source, 'CARMA_DU_SRC', rc=status)
      VERIFY_(STATUS)
...
!    Do the emission calculation
     ...
     allocate(radius(reg%NBIN),stat=STATUS)
     VERIFY_(STATUS)

     call CARMAGROUP_Get(gcCARMA%carma, igroup, rc, r=radius)
!    Update tracer mixing ratio and emissions diagnostic
     do ibin = 1, reg%NBIN
      emissions = 0.
      dqa = 0.
      n = n1 + (ielem-1)*reg%NBIN + ibin - 1
!     Take radius from cm to m
      radius_m = radius(ibin) * 1.e-2
      call DustEmissionGOCART( i1, i2, j1, j2, km, radius_m, &
                               fraclake, gwettop, oro, u10m, v10m, &
                               emissions, rc )
      dqa = reg%dust_emissions_fudgefactor * 1.e-9 * &
            dM(ibin) * gcCARMA%dust_source * emissions &
                     * dtime * grav_mks / (ple(:,:,km)-ple(:,:,km-1))
      qa(n)%data3d(:,:,km) = qa(n)%data3d(:,:,km) + dqa
      if( associated(DU_emis)) &
       DU_emis = DU_emis &
               + reg%dust_emissions_fudgefactor * 1.e-9 * dM(ibin)*gcCARMA%dust_source*emissions
     end do
     deallocate(radius)
    endif  ! Dust
!   ------------------------------------------------------------------------
...
!   ------------------------------------------------------------------------
    if(groupname == 'blackcarbon' .OR. groupname == 'BLACKCARBON') then
     ...
     if(gcCARMA%nymd_bc .ne. nymd) then
      ...
      if( .not. associated(gcCARMA%blackcarbon_emissions)) then
        allocate(gcCARMA%blackcarbon_emissions(i1:i2,j1:j2),stat=STATUS)
        VERIFY_(STATUS)
      endif

      if( associated(BC_emis)) BC_emis(:,:) = 0.
      allocate(biomass_src(i1:i2,j1:j2), biofuel_src(i1:i2,j1:j2), bc_ship_src(i1:i2,j1:j2), &
               ebcant1_src(i1:i2,j1:j2), ebcant2_src(i1:i2,j1:j2), stat=STATUS)
      VERIFY_(STATUS)
      biomass_src = 0.
      biofuel_src = 0.
      bc_ship_src = 0.
...
!     Get new source values
      nymd1 = nymd
      nhms1 = 120000

!     Source file reads go here as MAPL_GetPointer

!     For now, put emissions in first layer
      emissions = biomass_src + biofuel_src + ebcant1_src + ebcant2_src + bc_ship_src
      gcCARMA%blackcarbon_emissions = emissions

      deallocate(biomass_src, biofuel_src, bc_ship_src, ebcant1_src, ebcant2_src, stat=STATUS)
      VERIFY_(STATUS)

     endif

     if(associated(BC_emis)) BC_emis = gcCARMA%blackcarbon_emissions

     do ibin = 1, reg%NBIN
      n = n1 + (ielem-1)*reg%NBIN + ibin - 1
      dqa = dMbc(ibin) * gcCARMA%blackcarbon_emissions * dtime * grav_mks / (ple(:,:,km)-ple(:,:,km-1))
      qa(n)%data3d(:,:,km) = qa(n)%data3d(:,:,km) + dqa
     enddo
...
!   Smoke
!   ------------------------------------------------------------------------
!   PRC: For the moment we will assume Jamie's FINN smoke emissions
    if ( groupname == 'smoke' .OR. groupname == 'SMOKE' ) then

!     Read smoke source function from file if not already allocated

      if( .not. associated(gcCARMA%smoke_source)) then
        allocate(gcCARMA%smoke_source(i1:i2,j1:j2),stat=STATUS)
        VERIFY_(STATUS)
      endif

      if( .not. associated(gcCARMA%smoke_altitude)) then
        allocate(gcCARMA%smoke_altitude(i1:i2,j1:j2,1:km),stat=STATUS)
        VERIFY_(STATUS)
      endif
 
      gcCARMA%smoke_source = 0.
      gcCARMA%smoke_altitude = 0.
...
!     Do the emission calculation
      ...
      allocate(radius(reg%NBIN), radius_real(reg%NBIN), &
               dr(reg%NBIN), dr_real(reg%NBIN), &
               rhop(reg%NBIN), rhop_real(reg%NBIN), stat=STATUS)
      VERIFY_(STATUS)
      call CARMAGROUP_Get(gcCARMA%carma, igroup, rc, r=radius, dr=dr)
      call CARMAELEMENT_Get(gcCARMA%carma, ielem, rc, rho=rhop)
      radius_real = radius
      dr_real = dr
      rhop_real = rhop

      ibinfirst = n1 + (ielem-1)*reg%NBIN
      ibinlast = ibinfirst + reg%NBIN - 1

      call SmokeEmission ( i1, i2, j1, j2, km, &
                           nymd, nhms, cdt, &
                           rhoa, ple, &
                           gcCARMA%smoke_source, &
                           gcCARMA%smoke_altitude, &
                           ibinfirst, ibinlast, &
                           radius_real, &
                           dr_real, &
                           rhop_real, &
                           pblh, &
                           zle, &
                           gcCARMA%lons, gcCARMA%lats, &
                           qa, rc )

      if ( MAPL_AM_I_ROOT() ) then
        write(*,*) 'JAS: Exited SmokeEmission'
      endif

      deallocate(radius, radius_real, dr, dr_real, rhop, rhop_real, stat=STATUS )
      VERIFY_(STATUS)
...
!      Update tracer mixing ratio and emissions diagnostic
       do ibin = 1, reg%NBIN
        ...
        qa(n)%data3d(i,j,:) =  qa(n)%data3d(i,j,:) &
                             + dM(ibin)*dtime*grav_mks/delp*point_column_emissions/area(i,j)
        if( associated(ASH_emis)) &
         ASH_emis(i,j) = ASH_emis(i,j) + dM(ibin)*sum(point_column_emissions)/area(i,j)
       enddo
      enddo
   ...
   if(reg%NGAS > 0) then
     do igas = 1, reg%NGAS
      ...
      if( (trim(reg%gasname(igas)) == 'h2so4' .or. trim(reg%gasname(igas)) == 'H2SO4') .and. &
         reg%GOCART_H2SO4) then
       if(associated(pso4g_s))  &
         qa(n)%data3d = qa(n)%data3d + (pso4g_s+pso4wt_s+pso4aq_s) * dtime
       if(associated(pso4g_v))  &
         qa(n)%data3d = qa(n)%data3d + (pso4g_v+pso4wt_v+pso4aq_v) * dtime
!       if(associated(pso4g_t))  &
!         qa(n)%data3d = qa(n)%data3d + (pso4g_t+pso4wt_t+pso4aq_t) * dtime
      endif
     end do
   endif
...
!  Locals
   ...
   mie%sm_optics_file = reg%sm_optics_file
   mie%su_optics_file = reg%su_optics_file

!  Allocate and fill Mie tables
!   allocate( mie%mie_DU, mie%mie_SS, mie%mie_BC, mie%mie_SM, stat=STATUS)
   allocate( mie%mie_DU, mie%mie_SS, mie%mie_BC, mie%mie_SU, stat=STATUS)
   VERIFY_(STATUS)
   ...
   if ( rc /= 0 ) return
!   mie%mie_SM = Chem_MieTableCreate(mie%sm_optics_file, rc)
!   if ( rc /= 0 ) return
   ...
   if ( rc /= 0 ) return
!   call Chem_MieTableRead(mie%mie_SM,mie%nch,mie%channels,rc,nmom=mie%nmom)
!   if ( rc /= 0 ) return
...
!  Locals
   ...
   call Chem_MieTableDestroy(mie%mie_DU, rc=STATUS)
   VERIFY_(STATUS)
   call Chem_MieTableDestroy(mie%mie_SS, rc=STATUS)
   VERIFY_(STATUS)
   call Chem_MieTableDestroy(mie%mie_BC, rc=STATUS)
   VERIFY_(STATUS)
!   call Chem_MieTableDestroy(mie%mie_SM, rc=STATUS)
!   verify_(STATUS)
   call Chem_MieTableDestroy(mie%mie_SU, rc=STATUS)
   VERIFY_(STATUS)

!   deallocate(mie%mie_DU, mie%mie_SS, mie%mie_BC, mie%mie_SM, stat=STATUS)
   deallocate(mie%mie_DU, mie%mie_SS, mie%mie_BC, mie%mie_SU, stat=STATUS)
   VERIFY_(STATUS)

   deallocate(mie%vname, mie%vindex, mie%vtable, mie%channels, stat=STATUS)
   VERIFY_(STATUS)

   deallocate(mie, stat=STATUS)
