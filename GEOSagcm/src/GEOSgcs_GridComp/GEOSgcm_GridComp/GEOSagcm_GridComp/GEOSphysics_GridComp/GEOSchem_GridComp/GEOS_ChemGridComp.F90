          ...
          IF(myState%enable_GOCART) then
              CALL MAPL_AddConnectivity ( GC, &
                   SRC_NAME  = (/ 'GOCART::du001   ', 'GOCART::du002   ', 'GOCART::du003   ', 'GOCART::du004   ', 'GOCART::du005   ', &
                                  ...
                                  'GOCART::OCphobic', 'GOCART::OCphilic', 'GOCART::BCphobic', 'GOCART::BCphilic', &
                                  'GOCART::SO4     ' /), & 
                   DST_NAME  = (/ 'du001   ', 'du002   ', 'du003   ', 'du004   ', 'du005   ', &
                                  'ss001   ', 'ss002   ', 'ss003   ', 'ss004   ', 'ss005   ', &
                                  'ni001   ', 'ni002   ', 'ni003   ', &
                                  'OCphobic', 'OCphilic', 'BCphobic', 'BCphilic', &
                                  'SO4     ' /), &
      ...
      if(myState%enable_GOCART) then
       if(chemReg%doing_SU) then
         do n = chemReg%i_SU, chemReg%j_SU
            if(trim(chemReg%vname(n)) .eq. 'SO4') &
               CALL MAPL_AddConnectivity ( GC, &
                    SHORT_NAME  = (/'PSO4G  ', 'PSO4WET', 'PSO4AQ '/), &
                    DST_ID=CARMA, SRC_ID=GOCART, __RC__)
            if(trim(chemReg%vname(n)) .eq. 'SO4v') &
               CALL MAPL_AddConnectivity ( GC, &
                    SHORT_NAME  = (/'PSO4Gvolc  ', 'PSO4WETvolc', 'PSO4AQvolc '/), &
                    DST_ID=CARMA, SRC_ID=GOCART, __RC__)
            if(trim(chemReg%vname(n)) .eq. 'SO4s') &
               CALL MAPL_AddConnectivity ( GC, &
                    SHORT_NAME  = (/'PSO4Gstrat  ', 'PSO4WETstrat', 'PSO4AQstrat '/), &
                    DST_ID=CARMA, SRC_ID=GOCART, __RC__)
         enddo
       endif
      endif 
