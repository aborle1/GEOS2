...
! !INTERNAL STATE:
    ...
    OPTIONAL_VOC_INTERNAL: if (self%voc_chem) then
        ...
        call MAPL_AddInternalSpec(GC,                                                  &
                                  ...
                                  LONG_NAME  = 'Volatile Organic Compound (VOC)',      &
                                  UNITS      = 'mol mol-1',                            &
                                  DIMS       = MAPL_DimsHorzVert,                      &
                                  VLOCATION  = MAPL_VLocationCenter,                   &
                                  FRIENDLYTO = 'DYNAMICS:TURBULENCE:MOIST',            &
...
!  -------------
   ...
   real, pointer, dimension(:,:,:) :: q_VOC
   real, pointer, dimension(:,:,:) :: q_OA
   real, pointer, dimension(:,:,:) :: q_OAmmrd
...
!  ----------------
   ...
   real, allocatable, dimension(:,:,:) :: dOA
...
!  -------------
   ...
   call MAPL_GetPointer(export, q_OA,           'pSOA_ANTHRO_VOC',         __RC__)
   call MAPL_GetPointer(export, q_OAmmrd,       'pSOA_ANTHRO_VOC_MMRday',  __RC__)
...
!  -------------
   ...
   if (self%voc_chem) then
       call MAPL_GetPointer(internal, q_VOC, trim(comp_name)//'::'//'VOC', __RC__)
   end if
   ...
   if (self%gas_phase_chem) then
       ...
       OPTIONAL_VOC_CHEMISTRY: if (self%voc_chem) then
         allocate(dVOC(i1:i2,j1:j2,1:km), &
                  dOA(i1:i2,j1:j2,1:km), &
                  rk_OA_OH(i1:i2,j1:j2,1:km),__STAT__)

         where (q_VOC<tiny(0.0)) q_VOC = tiny(0.0)

         ! rate coefficient from Kim et al 2015
         rk_OA_OH = 1.25d-11*N_avog*q_OH_*density_air/mw_air*(1.0e-6)*cdt
         dVOC     = q_VOC*(1.0-exp(-rk_OA_OH))  ! Loss of VOC (mol/mol air)
         dOA      = dVOC                        ! Production of OA (mol/mol air)
         q_VOC    = q_VOC- dVOC                 ! Update VOC (mol/mol air)

         where (q_VOC<tiny(0.0)) q_VOC = tiny(0.0)

         dOA      = dOA*self%soa_MW/mw_air/cdt  ! kg kg-1 s-1

         if (associated(q_OAmmrd))q_OAmmrd = dOA*86400.0      ! kg kg-1 day-1

         if (associated(q_OA))    q_OA     = dOA*density_air  ! kg m-3 s-1

        deallocate(dVOC, dOA, rk_OA_OH, __STAT__)
