...
! ---------------------
   ...
   TYPE(ESMF_Alarm)                  :: ReplayIsActive
   LOGICAL                           :: doingReplayNow
...
! Is replay running?
! ------------------
   doingReplayNow = .FALSE.
   CALL ESMF_ConfigGetAttribute(CF, ReplayMode, Label='REPLAY_MODE:', DEFAULT="NoReplay", RC=STATUS)

   IF(ReplayMode == "Regular") THEN
    CALL ESMF_ClockGetAlarm(CLOCK, "ReplayActive", ReplayIsActive, RC=STATUS)

    IF(STATUS == 0) THEN
     doingReplayNow = ESMF_AlarmIsRinging(ReplayIsActive, RC=STATUS)
     VERIFY_(STATUS)
     IF(MAPL_AM_I_ROOT()) PRINT *,TRIM(Iam)//": Replay detection: ",doingReplayNow
    END IF
...
! ----------------------------------------
   gcGMI%gcEmiss%doingReplayNow = doingReplayNow
