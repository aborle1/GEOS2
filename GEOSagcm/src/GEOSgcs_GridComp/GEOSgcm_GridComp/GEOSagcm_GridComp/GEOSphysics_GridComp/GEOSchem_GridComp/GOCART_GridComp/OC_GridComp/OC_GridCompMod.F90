...
!-------------------------------------------------------------------------
   ...
   integer :: i, j, k, nymd1, nhms1, ijl, ijkl, ijk1l
   real :: qmax, qmin
   real :: qUpdate, delq
...
!   ----------------------------------------------
    ...
    call MAPL_GetPointer(impChem,var2d,'OC_BIOMASS'//iNAME,rc=status)
    VERIFY_(STATUS)
...
!   ----------------------------------------------------------
    call MAPL_GetPointer(impChem,var2d,'OC_TERPENE'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%terpene_src = var2d

    call MAPL_GetPointer(impChem,var2d,'OC_BIOFUEL'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%biofuel_src = var2d

    call MAPL_GetPointer(impChem,var2d,'OC_ANTEOC1'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%eocant1_src = var2d

    call MAPL_GetPointer(impChem,var2d,'OC_ANTEOC2'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%eocant2_src = var2d

!   Ship based OC emissions
    call MAPL_GetPointer(impChem,var2d,'OC_SHIP'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%oc_ship_src = var2d

!   Aircraft emissions during the three phases of flight
    call MAPL_GetPointer(impChem,var2d,'OC_AVIATION_LTO'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%aviation_lto_src = var2d

    call MAPL_GetPointer(impChem,var2d,'OC_AVIATION_CDS'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%aviation_cds_src = var2d

    call MAPL_GetPointer(impChem,var2d,'OC_AVIATION_CRS'//iNAME,rc=status)
    VERIFY_(STATUS)
    gcOC%aviation_crs_src = var2d
    
!   SOA production from oxidation of anthropogenic VOC
    call MAPL_GetPointer(impChem,var3d,'pSOA_ANTHRO_VOC'//iNAME,rc=status)
    VERIFY_(STATUS)
...
!   As a safety check, where value is undefined set to 0
    do j = j1, j2
     ...
     enddo
    enddo
    where( 1.01 * gcOC%psoa_anthro_voc .gt. undefval) gcOC%psoa_anthro_voc = 0.
...
!     Convert kg m-3 s-1 do kg m-2 s-1
!     -------------------------------------------
      srcSOAanthro =  gcOC%psoa_anthro_voc(i,j,k) * w_c%delp(i,j,k)/rhoa(i,j,k)/grav
      srcHydrophilic(i,j) = srcHydrophilic(i,j) + srcSOAanthro
