...
!   Dust
!   ---
    if ( ChemReg%doing_DU .and. .not. myState%data_driven) then
...
!      --------------------------------------
       call AddFromExportToBundle_(expChem, bundle, 'DUSV001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'DUSV002', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'DUSV003', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'DUSV004', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'DUSV005', __RC__)
...
!   Black Carbon
!   ------------
    if ( ChemReg%doing_BC .and. .not. myState%data_driven) then
...
!      Wet deposition (Convective scavenging)
!      --------------------------------------
       call AddFromExportToBundle_(expChem, bundle, 'BCSV001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'BCSV002', __RC__)
...
!      Gravitational Settling
!      ----------------------
       call AddFromExportToBundle_(expChem, bundle, 'BCSD001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'BCSD002', __RC__)

    end if

!   Organic Carbon
!   --------------
    if ( ChemReg%doing_OC .and. .not. myState%data_driven) then
...
!      Wet deposition (Convective scavenging)
!      --------------------------------------
       call AddFromExportToBundle_(expChem, bundle, 'OCSV001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'OCSV002', __RC__)
...
!      Gravitational Settling
!      ----------------------
       call AddFromExportToBundle_(expChem, bundle, 'OCSD001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'OCSD002', __RC__)

    end if

!   Sulfate (SO4; only aerosol component; bin 003)
!   ----------------------------------------------
    if ( ChemReg%doing_SU .and. .not. myState%data_driven) then

!      Dry deposition
!      --------------
       call AddFromExportToBundle_(expChem, bundle, 'SUDP003', __RC__)

!      Wet deposition (Convective scavenging)
!      --------------
       call AddFromExportToBundle_(expChem, bundle, 'SUSV003', __RC__)

!      Wet deposition
!      --------------
       call AddFromExportToBundle_(expChem, bundle, 'SUWT003', __RC__)

!      Gravitational Settling
!      ----------------------
       call AddFromExportToBundle_(expChem, bundle, 'SUSD003', __RC__)

    end if

!   Sea Salt
!   --------
    if ( ChemReg%doing_SS .and. .not. myState%data_driven) then

!      Dry deposition
!      --------------
       call AddFromExportToBundle_(expChem, bundle, 'SSDP001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSDP002', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSDP003', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSDP004', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSDP005', __RC__)

!      Wet deposition (Convective scavenging)
!      --------------------------------------
       call AddFromExportToBundle_(expChem, bundle, 'SSSV001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSV002', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSV003', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSV004', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSV005', __RC__)

!      Wet deposition
!      --------------
       call AddFromExportToBundle_(expChem, bundle, 'SSWT001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSWT002', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSWT003', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSWT004', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSWT005', __RC__)

!      Gravitational Settling
!      ----------------------
       call AddFromExportToBundle_(expChem, bundle, 'SSSD001', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSD002', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSD003', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSD004', __RC__)
       call AddFromExportToBundle_(expChem, bundle, 'SSSD005', __RC__)
...
!  Get the diagnostics
   ...
   if(w_c%reg%doing_ni) then
     ...
     if(associated(pm)   .and. associated(nismass))   pm   = pm   + (80.043/62.0)*nismass
     if(associated(pm25) .and. associated(nismass25)) pm25 = pm25 + (80.043/62.0)*nismass25
   endif
    ...
    subroutine aap_(mode, q, num, diameter, sigma, f_dust, f_soot, f_organic, dens_, q_, &
     ...
     case ('sulforg01')  !different distributions for clean and polluted environments
         where (q > max_clean)          
             sigma    = SIGI(1)
             diameter = DPGI(1)
             num      = TPI(1) * q_ / (dens_*fmassaux)             ! only sulfate  mass
         elsewhere 
             sigma    = SIGIclean(1)
             diameter = DPGIclean(1)
             num      = TPIclean(1) * q_ / (dens_*fmassclean)      ! only sulfate 
         end where 
     ...
     case ('sulforg02')
         where (q > max_clean)
             sigma    = SIGI(2)
             diameter = DPGI(2)
             num      = TPI(2) * q_ / (dens_*fmassaux)            ! only sulfate mass
         elsewhere
             sigma    = SIGIclean(2)
             diameter = DPGIclean(2)        
             num      = TPIclean(2) * q_ / (dens_*fmassclean)     ! only sulfate
         end where 
     ...
     case ('sulforg03')
         where (q > max_clean)
             sigma    = SIGI(3)
             diameter = DPGI(3)        
             num      = TPI(3) * q_ / (dens_*fmassaux)           ! only sulfate mass
         elsewhere 
             sigma    = SIGIclean(3)
             diameter = DPGIclean(3)
             num      = TPIclean(3) * q_ / (dens_*fmassclean)    ! only sulfate
         end where 
