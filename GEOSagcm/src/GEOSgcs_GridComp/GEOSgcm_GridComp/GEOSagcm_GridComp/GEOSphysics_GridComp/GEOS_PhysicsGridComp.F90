...
! This will call the stage 1 run routines of all chemistry components that have 
! two or more run phases (ckeller, 08/29/2014)
!------------------------------------------------------------------------------

! ckeller - Set TH for chemistry to TH after MOIST 
    if ( SYNCTQ.eq.1. ) then
     THFORCHEM = THAFMOIST 
    endif
...
!---------------------------------------------------------------
    ...
    if( DPEDT_PHYS ) then
       ...
       do l=1,lm
          ...
          !  Residual Tracer METHOD
          !  ----------------------
             dwdt =              QV(:,:,L) ! - DT*DQVDTCHM(:,:,L)  We do not keep dry-mass budget for CHEM constituents
             dwdt = dwdt +     QLLS(:,:,L)
             dwdt = dwdt +     QLCN(:,:,L)
             dwdt = dwdt +     QILS(:,:,L)
             dwdt = dwdt +     QICN(:,:,L)
