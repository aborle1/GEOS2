...
module GEOS_MoistGridCompMod
  ...
  use CLOUDNEW, only: &
       ...
       DZET_DEV, QDDF3_DEV, TEMPOR_DEV, CNV_FRACTION_DEV, &
       ! Inoutputs
...
!!$       CURAINMOVE_DEV, CUSNOWMOVE_DEV, &
       ...
       MAX_RI, RI_ANV, PDFFLAG, MOVE2RAS, ICE_SETTLE
  use cudafor
#endif
...
!!! downdraft diags
    ...
    call MAPL_AddExportSpec(GC,                           &
         SHORT_NAME ='NONDUST_IMM',                                       &
         LONG_NAME  ='Nondust Immersion IN', &
         UNITS      ='m-3',                                          &
         DIMS       = MAPL_DimsHorzVert,                           &
         VLOCATION  = MAPL_VLocationCenter,             RC=STATUS  )  
    VERIFY_(STATUS)
    ...
    call MAPL_AddExportSpec(GC,                           &
         SHORT_NAME ='NONDUST_DEP',                                       &
         LONG_NAME  ='Nondust deposition IN', &
         UNITS      ='m-3',                                          &
         DIMS       = MAPL_DimsHorzVert,                           &
         VLOCATION  = MAPL_VLocationCenter,             RC=STATUS  )  
    VERIFY_(STATUS)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine MOIST_DRIVER(IM,JM,LM, RC)
      ...
      real, pointer, dimension(:,:  ) :: RASPBLQ, RASTIME, MXDIAM, RH600, Q600, QRATIO, CNV_FRC
...
!!$      real, pointer, dimension(:,:,:) :: CURAINMOVE, CUSNOWMOVE
      ...
      real, pointer, dimension(:,:,:) :: SMAXL, WSUB, CCN01, CCN04, CCN1, SMAXI, & !DONIF
           ...
           NHET_IMM, NHET_DEP, NHET_DHF, &
           DUST_IMM, DUST_DEP, DUST_DHF, SCF, SCF_ALL, SIGW_GW, SIGW_CNV, & 
           SIGW_TURB, SIGW_RC, CNV_FICE, &
           CNV_NDROP, CNV_NICE, RHCmicro, DNHET_IMM,  NONDUST_IMM, &
           NONDUST_DEP, BERG, BERGS, MELT,  DNHET_CT, DTDT_macro, QCRES, DT_RASP, &
           ...
           DNDCNV, DNCCNV, DTDT_moist, DTDTCN
      ...
      real, pointer, dimension(:,:) ::  CLDREFFI_TOP, CLDREFFL_TOP, NCPL_TOP, NCPI_TOP, QCVAR_EXP, EIS, LTS


      real, allocatable, dimension(:,:,:)  ::  FQAl, FQAI, QCNTOT, FQA
      real, dimension(IM,JM,LM)  ::  QTOT, QL_TOT, QI_TOT, CFX, QRAIN_CN, QSNOW_CN, CNV_NDROP_X, CNV_NICE_X
...
!!! mmicro_pcond real*8 scalars and arrays 
      ...
      real(ESMF_KIND_R8), dimension(1,1:LM)        ::    & 
           ...
           sc_icer8, nhet_immr8, dnhet_immr8, nhet_depr8, nhet_dhfr8, &  !DONIF     
           dust_immr8, dust_depr8, dust_dhfr8, dpre8, npre8, mnuccdor8, nnucctor8, &
      ...
      real(ESMF_KIND_R8)                     :: ncnstr8,  ninstr8, disp_liu, ui_scale, & 
           dcrit, tfreez, qcvar8, ts_autice, dcsr8, qcvarr8
      ...
      real                               ::  T_ICE_ALL , T_ICE_MAX, USURF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real                            :: PMIN_DET,AUTOC_CN_LAND,AUTOC_CN_OCN, LCCIRRUS, & 
           ...
           EIS_LOW, EIS_UP, MIN_EXP, BKGTAU, DCRIT_, USE_AV_V, TS_AUTO_ICE, CCN_PARAM, IN_PARAM, &
      ...
      logical ALLOC_NHET_DHF
      logical ALLOC_DUST_IMM
      logical ALLOC_DUST_DEP
      logical ALLOC_DUST_DHF
      logical ALLOC_SCF
      ...
      logical ALLOC_NONDUST_IMM
      logical ALLOC_NONDUST_DEP
      logical ALLOC_BERG
      logical ALLOC_BERGS
      logical ALLOC_MELT
...
!!$      logical :: COPY_CUSNOWMOVE
      ...
      call MAPL_GetResource(STATE, EIS_LOW,        'EIS_LOW:',        DEFAULT= 0.0,   RC=STATUS) !lower LTS for morphology correction
      call MAPL_GetResource(STATE, EIS_UP,         'EIS_UP:',         DEFAULT= 5.0,   RC=STATUS) !Upper LTS for morphology correction
      call MAPL_GetResource(STATE, MIN_EXP,        'MIN_EXP:',        DEFAULT= 1.0,   RC=STATUS) !Exponent of the relation CFA=CFV^n
      call MAPL_GetResource(STATE, DCRIT_,         'DCRIT_DRIZZLE:',  DEFAULT= 0.5,    RC=STATUS) !scale factor for critical size for drizzle
      call MAPL_GetResource(STATE, USE_AV_V,       'USE_AV_V:',       DEFAULT= 0.0,    RC=STATUS) !Set to > 0 to use an average velocity for activation
      call MAPL_GetResource(STATE, TS_AUTO_ICE,    'TS_AUTO_ICE:',    DEFAULT= 1.5, RC=STATUS) !Ice autoconversion time scale
      call MAPL_GetResource(STATE, TMAXLL,         'TMAXLL:',         DEFAULT= 296.0,  RC=STATUS) !Ice autoconversion time scale
      call MAPL_GetResource(STATE, CCN_PARAM,      'CCNPARAM:',       DEFAULT= 2.0,    RC=STATUS) !CCN activation param
      call MAPL_GetResource(STATE, IN_PARAM,       'INPARAM:',        DEFAULT= 5.0,    RC=STATUS) !IN param
      ...
      call MAPL_GetResource(STATE, SIGMA_NUC,      'SIGMA_NUC:',      DEFAULT= 0.28,   RC=STATUS) !Widht of the in-cloud distribution of relative humidity in cirrus
...
!!!!!!!!!!!!!!!!!
      ...
      if( imsize.le.200       ) call MAPL_GetResource(STATE, RASPARAMS%RASAL2, 'RASAL2:', DEFAULT= -43200.0 ,RC=STATUS)
      if( imsize.gt.200 .and. &
          imsize.le.400       ) call MAPL_GetResource(STATE, RASPARAMS%RASAL2, 'RASAL2:', DEFAULT= -43200.0 ,RC=STATUS)
      if( imsize.gt.400 .and. &
          imsize.le.800       ) call MAPL_GetResource(STATE, RASPARAMS%RASAL2, 'RASAL2:', DEFAULT= -43200.0 ,RC=STATUS)
      if( imsize.gt.800 .and. &
          imsize.le.1600      ) call MAPL_GetResource(STATE, RASPARAMS%RASAL2, 'RASAL2:', DEFAULT= -64800.0 ,RC=STATUS)
      if( imsize.gt.1600      ) call MAPL_GetResource(STATE, RASPARAMS%RASAL2, 'RASAL2:', DEFAULT= -86400.0 ,RC=STATUS)
      ...
      call MAPL_GetResource(STATE, RASPARAMS%RASAL_EXP , 'RASAL_EXP:',  DEFAULT= 1  ,  RC=STATUS)
      ...
      if( imsize.le.800       ) call MAPL_GetResource(STATE, CBL_TPERT, 'CBL_TPERT:', DEFAULT=  -1.0 ,RC=STATUS)
      if( imsize.gt.800 .and. &
          imsize.le.1600      ) call MAPL_GetResource(STATE, CBL_TPERT, 'CBL_TPERT:', DEFAULT=  -0.5 ,RC=STATUS)
      if( imsize.gt.1600      ) call MAPL_GetResource(STATE, CBL_TPERT, 'CBL_TPERT:', DEFAULT=   0.0 ,RC=STATUS)
...
!!$      call MAPL_GetPointer(EXPORT, CUSNOWMOVE,'CUSNOWMOVE', RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, NHET_DHF, 'NHET_DHF'      , RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, DUST_DHF, 'DUST_DHF'      , RC=STATUS); VERIFY_(STATUS)
      ...
      call MAPL_GetPointer(EXPORT, NONDUST_IMM, 'NONDUST_IMM'      , RC=STATUS); VERIFY_(STATUS)
      call MAPL_GetPointer(EXPORT, NONDUST_DEP, 'NONDUST_DEP'      , RC=STATUS); VERIFY_(STATUS)
      ...
      if(adjustl(CLDMICRO)=="2MOMENT") then
         ...
         ALLOC_NHET_DHF     = .not.associated(NHET_DHF)
         ALLOC_DUST_IMM     = .not.associated(DUST_IMM)
         ALLOC_DUST_DEP     = .not.associated(DUST_DEP)
         ALLOC_DUST_DHF     = .not.associated(DUST_DHF)
         ALLOC_NONDUST_IMM     = .not.associated(NONDUST_IMM)
         ALLOC_NONDUST_DEP     = .not.associated(NONDUST_DEP)
         ...
         if(ALLOC_NHET_DHF  ) allocate (NHET_DHF   (IM,JM,LM))
         if(ALLOC_DUST_IMM  ) allocate (DUST_IMM   (IM,JM,LM))
         if(ALLOC_DUST_DEP  ) allocate (DUST_DEP   (IM,JM,LM))
         if(ALLOC_DUST_DHF  ) allocate (DUST_DHF   (IM,JM,LM))
         if(ALLOC_NONDUST_IMM  ) allocate (NONDUST_IMM   (IM,JM,LM))
         if(ALLOC_NONDUST_DEP  ) allocate (NONDUST_DEP   (IM,JM,LM))
    ...
    !    CNV_FRACTION = 1.0  --->  Deep-Convection
     ...
     !     associated with active deep convection lifting moisture to the mid-troposphere
      ...
      call MAPL_GetResource(STATE,CNV_Q600_MIN, 'CNV_Q600_MIN:', DEFAULT= 0.00250, RC=STATUS)
      VERIFY_(STATUS)
      call MAPL_GetResource(STATE,CNV_Q600_MAX, 'CNV_Q600_MAX:', DEFAULT= 0.00600, RC=STATUS)
      VERIFY_(STATUS)
      ...
      call cloud_ptr_stubs (SMAXL, SMAXI, WSUB, CCN01, CCN04, CCN1, &
            ...
            NHET_DHF, DUST_IMM, DUST_DEP, DUST_DHF, SCF, SCF_ALL, &
            SIGW_GW, SIGW_CNV, &
            SIGW_TURB, SIGW_RC, RHCmicro, DNHET_IMM, NONDUST_IMM, NONDUST_DEP, &
          ...
          if (n_modes > 0) then
            ...
            END IF
                  ...
                  do k = 1, LM
                      do j = 1, JM
                          do i = 1, IM
                              AeroProps(i,j,k)%num(n)   = aci_num(i,j,k)
                              AeroProps(i,j,k)%dpg(n)   = aci_dgn(i,j,k)
                              AeroProps(i,j,k)%sig(n)   = aci_sigma(i,j,k)        ! no need of a 3D field: mode specific
                              AeroProps(i,j,k)%kap(n)   = aci_hygroscopicity(i,j,k)
                              AeroProps(i,j,k)%den(n)   = aci_density(i,j,k)
                              AeroProps(i,j,k)%fdust(n) = aci_f_dust(i,j,k)
                              AeroProps(i,j,k)%fsoot(n) = aci_f_soot(i,j,k)
                              AeroProps(i,j,k)%forg(n)  = aci_f_organic(i,j,k)
...
! find the minimun level for cloud micro calculations
	 call find_l(KMIN_TROP, PLO, pmin_trop, IM, JM, LM, 10, LM-2) 
      ...
      if(associated(RASTIME )) RASTIME = RASAL2_2d
...
!      if(associated(TVQX1))  TVQX1     = SUM( (  Q1 +  QLLS + QLCN + QILS + QICN + CNV_PRC3)*DM + CNV_DQLDT*DT_MOIST , 3 )
      ...
      if( imsize.le.200       ) call MAPL_GetResource(STATE, CLDPARAMS%RAIN_REVAP_FAC, 'RAIN_REVAP_FAC:', DEFAULT= 1.0 ,RC=STATUS)
      if( imsize.gt.200 .and. &
          imsize.le.400       ) call MAPL_GetResource(STATE, CLDPARAMS%RAIN_REVAP_FAC, 'RAIN_REVAP_FAC:', DEFAULT= 1.0 ,RC=STATUS)
      if( imsize.gt.400 .and. &
          imsize.le.800       ) call MAPL_GetResource(STATE, CLDPARAMS%RAIN_REVAP_FAC, 'RAIN_REVAP_FAC:', DEFAULT= 1.0 ,RC=STATUS)
      if( imsize.gt.800 .and. &
          imsize.le.1600      ) call MAPL_GetResource(STATE, CLDPARAMS%RAIN_REVAP_FAC, 'RAIN_REVAP_FAC:', DEFAULT= 2.0 ,RC=STATUS)
      if( imsize.gt.1600      ) call MAPL_GetResource(STATE, CLDPARAMS%RAIN_REVAP_FAC, 'RAIN_REVAP_FAC:', DEFAULT= 3.0 ,RC=STATUS)
...
!     call MAPL_GetResource( STATE, CLDPARAMS%CNV_ENVF,       'CNV_ENVF:',       DEFAULT= 0.8     )
      ...
      if( imsize.gt.1600      ) call MAPL_GetResource( STATE, CLDPARAMS%MINRHCRIT, 'MINRHCRIT:', DEFAULT=0.97, RC=STATUS)
	 ...
	 call MAPL_GetResource( STATE, CLDPARAMS%TURNRHCRIT,     'TURNRHCRIT:',     DEFAULT= 884.0 )
         call MAPL_GetResource( STATE, CLDPARAMS%MOVE2RAS,       'MOVE2RAS:',       DEFAULT= 1.0   )
      else   
	 ...
	 call MAPL_GetResource( STATE, CLDPARAMS%TURNRHCRIT,     'TURNRHCRIT:',     DEFAULT= 750.0 )
      ...
      call MAPL_GetResource( STATE, CLDPARAMS%MIN_RL,    'MIN_RL:',     DEFAULT= 10.e-6 )
      call MAPL_GetResource( STATE, CLDPARAMS%MIN_RI,    'MIN_RI:',     DEFAULT= 20.e-6 )
      call MAPL_GetResource( STATE, CLDPARAMS%MAX_RL,    'MAX_RL:',     DEFAULT= 21.e-6 )
      call MAPL_GetResource( STATE, CLDPARAMS%MAX_RI,    'MAX_RI:',     DEFAULT= 40.e-6 )
      call MAPL_GetResource( STATE, CLDPARAMS%RI_ANV,    'RI_ANV:',     DEFAULT= 30.e-6 )
	 ...
	 do J=1,JM
		    ...
		    !ksa1= max(real(kbmin - kcldtopcvn), 1.0)
               ...
               ! ========================Activate the aerosols ============================================ 
                ...
                do K = KMIN_TROP(I, J), LM-2 !limit to troposphere and no activation at the surface
		       ...
		       call vertical_vel_variance(omegr8(1, K), lc_turb(1, K), ter8(1, K), plevr8(1, K), rad_cooling(1,K),  uwind_gw(1,K), &
						            ...
						            wparc_ls(1, K), wparc_gw(1, K), wparc_cgw(1, K), wparc_turb(1, K), EIS(I, J))
		     
	
					
	                 if (K .ge. kbmin-3) wparc_turb(1, K)=max(wparc_turb(1,K), 0.04)    !minimum velocity within the PBL (not resolved by RAS)
		     ...
		     !!Subroutine aerosol_activate contains the CCN activation and ice nucleation parameterizations. Lives in aer_cloud.F90.
                     ...
                     call   aerosol_activate(tauxr8, plevr8(1, K), swparc(1, K), wparc_ls(1, K),  AeroAux, &
                          ...
                          nhet_immr8(1, K), dnhet_immr8(1, K), nhet_depr8(1, k), nhet_dhfr8(1, k), sc_icer8(1, k), &
                          dust_immr8(1, K), dust_depr8(1, k), dust_dhfr8(1, k), nlimicer8(1, k), use_average_v, int(CCN_PARAM), int(IN_PARAM),  &
			  fdust_drop, fsoot_drop, pfrz_inc_r8(1, K), sigma_nuc_r8, rh1_r8)
...
!!!Call to MG microphysics. Lives in cldwat2m_micro.F90
              ...
              PFI_LS_X(I, J, 1:LM) = sflx(1, 1:LM)*FQA(I, J, 1:LM)
              PFI_AN_X(I, J, 1:LM) = sflx(1, 1:LM)*FQA(I, J, 1:LM)
               ...
               CLDREFFL(I,J,1:LM) = max(REAL(effcr8(1,1:LM))*1.0e-6, 1.0e-6) !Avoid type errors
               CLDREFFI(I,J,1:LM) = max(REAL(effir8(1,1:LM))*1.0e-6, 1.0e-6)      
               CLDREFFR(I,J,1:LM) = REAL(droutr8(1, 1:LM))/2.0        
	       CLDREFFS(I,J,1:LM) = REAL(dsoutr8(1, 1:LM))/2.0 
...
do K= 1, LM
   ...
   DIPDF_X=((QILS+QLCN)  - DIPDF_X)/DT_MOIST
 ...
 !if(associated(TVQX2)) TVQX2    = SUM( ( Q1 +  QLLS + QLCN + QILS + QICN )*DM , 3 )  + TPREC*DT_MOIST
	 ...
	 SEASALT=SEASALT*CFX
        ...
        !to m-3 s-1
         ...
         !make grid averages
         !NONDUST_IMM = (NHET_IMM-DUST_IMM)*CFICE
         !NONDUST_DEP = (NHET_NUC-DUST_DEP)*CFICE
         ...
         !----- Calculate low tropospheric stability
      ...
      !   DO I=1, IM
       !     DO J= 1 , JM
        ...
        !       DO K = LM-1, 2, -1 
        !          If (PLO(I, J, K) .lt. 500.0) then
        !             OMEGA500_X(I, J) =  OMEGA(I, J, K+1)*0.01*86400.0 !hpa/day
        !             exit
        !          end if
        !       END DO
  ...
  !       END DO
         ...
         DO I=1, IM
            DO J = 1, JM    
               ...
               !if ((TEMP(I, J, LM) .gt. 275.0) .and. (TEMP(I, J, LM) .lt. TMAXLL)) then 

               if ((TEMP(I, J, LM) .gt. 275.0)  .and. (FRLAND(I, J) .lt. 0.1)) then 
                  ...
                  DO K=LM-1, 2, -1
                     if ((RAD_CF(I, J, K) .gt. 0.01) .and. (RAD_CF(I, J, K) .lt. 0.99)) then  
                        ...
                        USURF=1.0
                        USURF= (EIS_UP-EIS(I, J))/(EIS_UP -  EIS_LOW) 
                        USURF=min(max(USURF, MIN_EXP), 1.0)
                        fracover=min(max((TEMP(I, J, K) -275.0)/5.0, 0.0), 1.0) 
         ...
         ! Diagnostic cloud top properties
         CLDREFFL_TOP_X = 0.0
         CLDREFFI_TOP_X = 0.0
         NCPL_TOP_X = 0.0
         NCPI_TOP_X = 0.0
         ...
         WHERE (QTOT .gt. 0.0)
            SCF=min(max(QL_TOT/QTOT, 0.0), 1.0)
         ELSEWHERE 
            SCF=0.0
         END WHERE
...
! --------------------------------------------------------------------------------------------
      ...
      if(adjustl(CLDMICRO)=="2MOMENT") then
         ...
         if(ALLOC_NHET_DHF) deallocate(NHET_DHF)
         if(ALLOC_DUST_IMM) deallocate(DUST_IMM)
         if(ALLOC_DUST_DEP) deallocate(DUST_DEP)
         if(ALLOC_DUST_DHF) deallocate(DUST_DHF)
         if(ALLOC_NONDUST_IMM) deallocate(NONDUST_IMM)
         if(ALLOC_NONDUST_DEP) deallocate(NONDUST_DEP)      
