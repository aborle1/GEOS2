...
#if 0
! Simple Nudging Based on Predictor Methodology
! ---------------------------------------------
   real, pointer, dimension(:,:,:)     ::  u_ana => null()
   real, pointer, dimension(:,:,:)     ::  v_ana => null()
   real, pointer, dimension(:,:,:)     ::  t_ana => null()
   real, pointer, dimension(:,:,:)     ::  p_ana => null()
...
! If so, overwrite increments from analysis by recreating them on the fly
! -----------------------------------------------------------------------
...
! ---------------------------------------------
    if(TYPE == CORRECTOR ) then
       ...
       call MAPL_GetPointer(IMPORT,ptr3d,'DUDT',RC=STATUS)
       VERIFY_(STATUS)
       allocate( u_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
       u_ana = ptr3d
       ptr3d = ptr3d - U

       call MAPL_GetPointer(IMPORT,ptr3d,'DVDT',RC=STATUS)
       VERIFY_(STATUS)
       allocate( v_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
       v_ana = ptr3d
       ptr3d = ptr3d - V

       call MAPL_GetPointer(IMPORT,ptr3d,'DTDT',RC=STATUS)
       VERIFY_(STATUS)
       allocate( t_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
       t_ana = ptr3d
       ptr3d = ptr3d - T

       call MAPL_GetPointer(IMPORT,ptr3d,'DPEDT',RC=STATUS)
       VERIFY_(STATUS)
       allocate( p_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
       p_ana = ptr3d
       ptr3d = ptr3d - PLE
	  ...
	  fieldName = MAPL_RmQualifier(STRING)
          if(TRIM(fieldName) == 'OX') then
             call MAPL_GetPointer(IMPORT,ptr3d,'DO3DT',RC=STATUS)
             VERIFY_(STATUS)
             allocate( o3_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
             o3_ana = ptr3d
             ptr3d = ptr3d - Q*1.e6
          endif
          if(NAMES(K)=='Q') then
             call MAPL_GetPointer(IMPORT,ptr3d,'DQVDT',RC=STATUS)
             VERIFY_(STATUS)
             allocate( q_ana(size(ptr3d,1),size(ptr3d,2),size(ptr3d,3)) )
             q_ana = ptr3d
             ptr3d = ptr3d - Q
          endif
       enddo
...
! ---------------------------------------------------------
    ...
    call MAPL_GetResource( STATE, POFFSET, Label="PREDICTOR_OFFSET:", default=21600. , RC=STATUS)
    VERIFY_(STATUS)
    nsteps=nint(POFFSET/DT)+1

    call MAPL_GetResource( STATE, FilterType, Label="FILTER_TYPE:", default="IAU" , RC=STATUS)
    VERIFY_(STATUS)
    FilterType = uppercase(FilterType)
    TNDcoeff = 1.0/TAUANL
...
!   Define coefficients to scale increments with
!   NOTE: this is placed here to all 3dIAU to use this feature as well
!   ------------------------------------------------------------------
    if (.not.associated(myCoeffs%dfi)) then
       allocate(myCoeffs%dfi(nsteps))
       myCoeffs%istep=0
       call dfi_coeffs (FilterType,DT,POFFSET,TAUANL,nsteps,myCoeffs%dfi) 
       if (MAPL_am_I_root()) then
          print*, 'DFI initialized for this many steps: ', nsteps
          do i=1,nsteps
             print*, 'i,LH-dfi-coeff=', i, myCoeffs%dfi(i),' (',1.0/(myCoeffs%dfi(i)*3600),' hrs)'
          enddo
       endif
    endif

    if (trim(FilterType)/="IAU") then
       myCoeffs%istep=myCoeffs%istep+1
       if(myCoeffs%istep<=nsteps) then
...
!   --------
    ...
    subroutine dfi_coeffs (FilterOpt,DT,TC,TAUANL,nsteps,dfi)
!   This subroutine belongs to GEOS_Shared, but for now it lives here
    ...
    real,   intent(in)  :: TAUANL ! IAU coefficient
...
!   --------------------------
    if (abs(TAUANL-TC) < 1.e-5) then
       if(MAPL_AM_I_ROOT()) then
         print *, 'Warning: Inconsistent IAU/filtering function'
         print *, 'Warning: Check TAUANL and POFFSET'
         print *, 'Warning: Ignoring TAUANL ...'
       endif
    endif
    pi = 4.0*atan(1.)
    nhlf = (nsteps+1)/2
