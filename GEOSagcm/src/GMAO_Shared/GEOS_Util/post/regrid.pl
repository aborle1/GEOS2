...
# global variables
#-----------------
my ($bcsHEAD, $bcsTagIN, $bcsTagOUT, $bkgFLG, $capture, $debug, $dbHash, $c2cX);
my ($drymassFLG, $ESMABIN, $ESMATAG, $expid, $gcmFLG, $g5modules, $getinput);
my ($getsponsorX, $grIN, $grINocean, $grOUT, $grOUTocean, $grpID, $grouplist);
...
my ($rst_template, $rstIN_template, $slurmjob, $scale_catchX, $scale_catchcnX);
my ($surfFLG, $surflay, $surflayIN, $tagIN, $tagOUT, $upairFLG, $verbose);
my ($workdir, $year, $ymd);
...
#------------------------------
foreach (qw/ 12 24 48 90 180 360 500 720 1000 1440 2000 2880 /) {
    $CS{"C$_"} = $_;
    $im{"C$_"} = $_;
    $jm{"C$_"} = 6*$_;
}
...
sub init_tag_arrays_and_hashes {
    ...
    # BCS Tags: Heracles-NL (New Land Parameters)
    #---------------------------------------------------------------------------
    @HNL  = qw( HNL Heracles-NL );
    ...
    foreach (@HNL)   { $landIceVER{$_} = 2; $bcsTAG{$_} = "Heracles-NL_Reynolds" }
    ...
    @GCMtags = (\@F14, \@F20, \@F21, \@G10, \@G10p, \@G20, \@G30, \@G40, \@HNL, \@ICA);
    ...
    %rank = ( "Icarus_Ostia"         => 20,
              "Icarus_MERRA-2"       => 19,
              "Icarus_Reynolds"      => 18,
              "Heracles-NL_Ostia"    => 17,
              "Heracles-NL_MERRA-2"  => 16,
              "Heracles-NL_Reynolds" => 15,
    ...
    #------------------------------------------
    $rank_1_catchcn = $rank{"Heracles-NL_Reynolds"};
    $rank_1_route = $rank{"Heracles-NL_Reynolds"};
}
...
sub check_inputs {
    ...
    # DAS mode defaults
    #------------------
    $fname = rstname($expid, "rst.lcv", $rstIN_template);
...
sub check_rst_files {
    ...
    if ($upairFLG) {
        foreach $type (keys(%UPPERAIR_REQ), keys(%UPPERAIR_OPT)) {
            ...
            else       { $notfound++ if $UPPERAIR_REQ{$type} }
        }
    ...
    if ($surfFLG) {
        foreach $type (sort keys %SURFACE) {
            ...
            if ($file) { $input_restarts{$type} = $file }
            else       { $notfound++ unless $type eq "catchcn_internal_rst" }
        }
        die "Exiting;" if $notfound;
    }
...
sub set_IN_OUT {
    ...
    foreach $HH (\%IN, \%iceIN, \%OUT) {
        ...
        if ($rank{$bcsTAG} >= $rank{"Icarus_Reynolds"}) {
            $bcsdir = "$bcsHEAD/Icarus/$bcsTAG/$gridID";
        }
        elsif ($rank{$bcsTAG} >= $rank{"Heracles-NL_Reynolds"}) {
            $bcsdir = "$bcsHEAD/Heracles-NL/$bcsTAG/$gridID";
        }
...
sub regrid_upperair_rsts_CS {
    ...
    $im = $im{$grOUT};
    if    ($im eq   "12") { $NPE =  12 }
    elsif ($im eq   "24") { $NPE =  12 }
    elsif ($im eq   "48") { $NPE =  12 }
    elsif ($im eq   "90") { $NPE =  12 }
    elsif ($im eq  "180") { $NPE =  24 }
    elsif ($im eq  "360") { $NPE =  96 }
    elsif ($im eq  "500") { $NPE =  96 }
    elsif ($im eq  "720") { $NPE = 192 }
    elsif ($im eq "1000") { $NPE = 384 }
    elsif ($im eq "1440") { $NPE = 576 }
    elsif ($im eq "2000") { $NPE = 768 }
    elsif ($im eq "2880") { $NPE = 768 } 
    else { die "Error; cannot recognize output grid: $grOUT;" }

    $QOS = "";
    $QOS = "SBATCH --qos=debug" if $qos eq "debug" and $NPE <= 532;
    $QOS = "SBATCH --qos=high"  if $im > 2000;
    ...
    $AGCMout   = rstnameI($workdir, "agcm_import_rst");
...
if (-e agcm_import_restart_in) then
  set c2cX = $c2cX
  set AGCMout = $AGCMout
  set grOUT = $grOUT
  $ESMABIN/esma_mpirun -np 1 \$c2cX agcm_import_restart_in \$AGCMout \$grOUT
endif
exit
...
sub regrid_surface_rsts {
    ...
    if ($landIceDT) {
        ...
        else                { $flags = "-saltwater -lake" }
    } else                  { $flags = "-saltwater -lake -landice" }
    ...
    #-----------------------------------------------
        ...
        foreach $type (@SFC) {
            ...
            if ($src) {
                if ($type eq "catchcn_internal_restart") {
    ...
    #------------------------------
    symlink_("\n$H2{bcsdir}/rst", $OutData_dir) if $mk_route;
    symlink_("\n$H2{bcsdir}/til", $OutData_dir) if $mk_route;
