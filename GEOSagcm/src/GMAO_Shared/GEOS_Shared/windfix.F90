...
#include "MAPL_Generic.h"
      ...
      k_index(L) =    (L-1)/npes + 1
      enddo
      kmax = (lm-1)/npes + 1
 
      allocate (  uglo(img,jmg,kmax) )
      allocate (  vglo(img,jmg,kmax) )
      allocate (  dglo(img,jmg,kmax) )
      allocate ( dpglo(img,jmg,kmax) )
...
! ---------------------------
      do L=1,lm
         k=k_index(L)
         call ArrayGather (local_array= ub(:,:,L),global_array= uglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
         call ArrayGather (local_array= vb(:,:,L),global_array= vglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
         call ArrayGather (local_array=dpb(:,:,L),global_array=dpglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
...
! --------------------------------
      do L=1,lm
         ...
         call getdiv (uglo(1,1,k),vglo(1,1,k),dpglo(1,1,k),dglo(1,1,k),img,jmg )               
         endif
      enddo
    ! call mpi_barrier (comm,ierror)
      ...
      do L=1,lm
         k=k_index(L)
         call ArrayScatter(local_array=divb(:,:,L), global_array=dglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
...
! -------------------------
      do L=1,lm
         k=k_index(L)
         call ArrayGather (local_array= ua(:,:,L),global_array= uglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
         call ArrayGather (local_array= va(:,:,L),global_array= vglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
         call ArrayGather (local_array=dpa(:,:,L),global_array=dpglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
...
! --------------------------------
    ...
    ! call mpi_barrier (comm,ierror)
      ...
      do L=1,lm
         k=k_index(L)
         call ArrayScatter(local_array=diva(:,:,L), global_array=dglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
...
! -----------------------------------------
      do L=1,lm
         k=k_index(L)
         call ArrayGather (local_array=diva(:,:,L),global_array=dglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
...
! ------------------------------------------------------------
      do L=1,lm
         k=k_index(L)
         if( index(L).eq.myid ) then
             call VELPOT_SP (dglo(1,1,k),chi,img,jmg)
             call gradq     (chi,  uchi,vchi,img,jmg)
             uglo(:,:,k) = uglo(:,:,k) + uchi(:,:)/dpglo(:,:,k)
             vglo(:,:,k) = vglo(:,:,k) + vchi(:,:)/dpglo(:,:,k)
         endif
      enddo
    ! call mpi_barrier (comm,ierror)
...
! -------------
      do L=1,lm
         k=k_index(L)
         call ArrayScatter (local_array=ua(:,:,L), global_array=uglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
         call ArrayScatter (local_array=va(:,:,L), global_array=vglo(:,:,k), depe=index(L), grid=GRIDana, rc=status)
         VERIFY_(STATUS)
      enddo
