...
!#ifdef 1
      ...
      subroutine get_ocn_fluxes(tauxb, tauyb)
      ...
      subroutine get_fresh_salt(fre, sal)
       ...
       type (block) :: &
        ...
        do iblk = 1, nblocks  
           ...
           do i = ilo, ihi
             ...
             j1 = j-nghost  
             if(tmask(i,j,iblk)) then
               fre(i1,j1) = fresh(i,j,iblk) 
               sal(i1,j1) = fsalt(i,j,iblk) 
             else
               fre(i1,j1) = 0.0 
               sal(i1,j1) = 0.0
             endif
      ...
      real (kind=real_kind), &
         dimension(:,:), optional, intent(out) :: &
          divuo, fakedivo, tauodivo, tauadivo, tau1divo
      real (kind=real_kind), &
         dimension(:,:), optional, intent(out) :: &
           strcorxo, strcoryo
